...
private[streaming] trait BlockGeneratorListener {
   ...
   * callback are synchronized with the block generation and its associated callback, so block
   * generation waits for the active data addition+callback to complete. This is useful for
   * updating metadata on successful buffering of a data item, specifically that metadata that
   * will be useful when a block is generated. Any long blocking operation in this callback will
   * hurt the throughput.
   */
  def onAddData(data: Any, metadata: Any): Unit

  /**
   * Called when a new block of data is generated by the block generator. The block generation and
   * this callback are synchronized with the data addition and its associated callback, so the
   * data addition waits for the block generation+callback to complete. This is useful for
   * updating metadata when a block has been generated, specifically metadata that will be useful
   * when the block has been successfully stored. Any long blocking operation in this callback
   ...
   * Spark in this method. Internally this is called from a single thread, that is not
   * synchronized with any other callbacks. Hence it is okay to do long blocking operation in this
   * callback.
   */
  def onPushBlock(blockId: StreamBlockId, arrayBuffer: ArrayBuffer[_]): Unit

  /**
   * Called when an error has occurred in the BlockGenerator. Can be called form many places so
   * better to not do any long block operation in this callback.
   */
  def onError(message: String, throwable: Throwable): Unit
}

/**
 * Generates batches of objects received by a [[org.apache.spark.streaming.receiver.Receiver]] and
 * puts them into appropriately named blocks at regular intervals. This class starts two threads,
 * one to periodically start a new batch and prepare the previous batch of as a block, the other
 * to push the blocks into the block manager.
 *
...
private[streaming] class BlockGenerator(
   ...
   *   - StoppedAddingData: stop() has been called, the adding of data has been stopped, but
   *     blocks are still being generated and pushed.
   *   - StoppedGeneratingBlocks: Generating of blocks has been stopped, but they are still being
   *     pushed.
  ...
  def stop(): Unit = {
    ...
    synchronized {
      if (state == Active) {
        ...
        logWarning(
          log"Cannot stop BlockGenerator as its not in the Active state " +
   ...
   * `BlockGeneratorListener.onAddData` callback will be called. Note that all the data items are
   * atomically added to the buffer, and are hence guaranteed to be present in a single block.
   */
