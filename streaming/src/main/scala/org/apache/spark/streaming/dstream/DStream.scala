 ...
 * org.apache.spark.rdd.RDD in the Spark core documentation for more details on RDDs). DStreams
 * can either be created from live data (such as, data from TCP sockets, Kafka, etc.) using a
 * [[org.apache.spark.streaming.StreamingContext]] or it can be generated by transforming existing
 * DStreams using operations such as `map`, `window` and `reduceByKeyAndWindow`. While a Spark
 * Streaming program is running, each DStream periodically generates a RDD, either from live data
 * or by transforming the RDD generated by a parent DStream.
 *
 ...
 * `join`. These operations are automatically available on any DStream of pairs (e.g.,
 * DStream[(Int, Int)] through implicit conversions.
 *
...
abstract class DStream[T: ClassTag](@transient private[streaming] var ssc: StreamingContext)
   ...
   *
   * This is not defined if the DStream is created outside of one of the public DStream
   * operations.
   */
   ...
   *
   * Each DStream produces many scopes and each scope may be shared by other DStreams created in
   * the same operation. Separate calls to the same DStream operation create separate scopes. For
   * instance, `dstream.map(...).map(...)` creates two separate scopes per batch.
   */
  ...
  /**
   * Initialize the DStream by setting the "zero" time, based on which the validity of future
   * times is calculated. This method also recursively initializes its parent DStreams.
   */
  private[streaming] def initialize(time: Time): Unit = {
    ...
    if (mustCheckpoint && checkpointDuration == null) {
      ...
      logInfo(
        log"Checkpoint interval automatically set to " +
  ...
  private[streaming] def remember(duration: Duration): Unit = {
    if (duration != null && (rememberDuration == null || duration > rememberDuration)) {
      ...
      logInfo(
        log"Duration for remembering RDDs set to " +
  ...
  private[streaming] def isTimeValid(time: Time): Boolean = {
    if (!isInitialized) {
      ...
      logInfo(
        log"Time ${MDC(LogKeys.TIME, time)} is invalid as zeroTime is " +
  ...
  /**
   * Get the RDD corresponding to the given time; either retrieve it from cache or
      ...
      if (isTimeValid(time)) {
        ...
        rddOption.foreach { case newRDD =>
          ...
          if (checkpointDuration != null && (time - zeroTime).isMultipleOf(checkpointDuration)) {
            newRDD.checkpoint()
            logInfo(
              log"Marking RDD ${MDC(LogKeys.RDD_ID, newRDD.id)} for time " +
  ...
  /**
   * Wrap a body of code such that the call site and operation scope information are passed to the
   * RDDs created in this body properly.
   * @param body
   *   RDD creation code to execute with certain local properties.
   * @param time
   *   Current batch time that should be embedded in the scope names
   * @param displayInnerRDDOps
   *   Whether the detailed callsites and scopes of the inner RDDs generated by `body` will be
   *   displayed in the UI; only the scope and callsite of the DStream operation that generated
  ...
  /**
   * Generate a SparkStreaming job for the given time. This is an internal method that should not
   * be called directly. This default implementation creates a job that materializes the
   * corresponding RDD. Subclasses of DStream may override this to generate their own jobs.
   */
  ...
  /**
   * Clear metadata that are older than `rememberDuration` of this DStream. This is an internal
   * method that should not be called directly. This default implementation clears the old
   * generated RDDs. Subclasses of DStream may override this to clear their own metadata along
   * with the generated RDDs.
   */
    ...
    if (unpersistData) {
      ...
      oldRDDs.values.foreach { rdd =>
        ...
        rdd match {
          case b: BlockRDD[_] =>
            logInfo(
              log"Removing blocks of RDD ${MDC(LogKeys.RDD, b)} " +
  ...
  /**
   * Refresh the list of checkpointed RDDs that will be saved along with checkpoint of this
   * stream. This is an internal method that should not be called directly. This is a default
   * implementation that saves only the file names of the checkpointed RDDs to checkpointData.
   * Subclasses of DStream (especially those of InputDStream) may override this method to save
  ...
  /**
   * Restore the RDDs in generatedRDDs from the checkpointData. This is an internal method that
   * should not be called directly. This is a default implementation that recreates RDDs from the
   * checkpoint file names stored in checkpointData. Subclasses of DStream that override the
   * updateCheckpointData() method would also need to override this method.
   */
  ...
  /**
   * Return a new DStream by applying a function to all elements of this DStream, and then
  ...
  /**
   * Return a new DStream in which each RDD is generated by applying glom() to each RDD of this
   * DStream. Applying glom() to an RDD coalesces all elements within each partition into an
   * array.
   */
   ...
   * of this DStream. Applying mapPartitions() to an RDD applies a function to each partition of
   * the RDD.
   */
  def mapPartitions[U: ClassTag](
      mapPartFunc: Iterator[T] => Iterator[U],
      preservePartitioning: Boolean = false): DStream[U] = ssc.withScope {
    new MapPartitionedDStream(this, context.sparkContext.clean(mapPartFunc), preservePartitioning)
  }

  /**
   * Return a new DStream in which each RDD has a single element generated by reducing each RDD of
   * this DStream.
   */
  ...
  /**
   * Return a new DStream in which each RDD has a single element generated by counting each RDD of
   * this DStream.
   */
  ...
  /**
   * Return a new DStream in which each RDD contains the counts of each distinct value in each RDD
   * of this DStream. Hash partitioning is used to generate the RDDs with `numPartitions`
   * partitions (Spark's default number of partitions if `numPartitions` not specified).
   */
  def countByValue(numPartitions: Int = ssc.sc.defaultParallelism)(implicit
      ord: Ordering[T] = null): DStream[(T, Long)] = ssc.withScope {
    this.map((_, 1L)).reduceByKey((x: Long, y: Long) => x + y, numPartitions)
  }

  /**
   * Apply a function to each RDD in this DStream. This is an output operator, so 'this' DStream
   ...
   *   foreachRDD function
   * @param displayInnerRDDOps
   *   Whether the detailed callsites and scopes of the RDDs generated in the `foreachFunc` to be
   *   displayed in the UI. If `false`, then only the scopes and callsites of `foreachRDD` will
   *   override those of the RDDs on the display.
   */
  private def foreachRDD(
      foreachFunc: (RDD[T], Time) => Unit,
      displayInnerRDDOps: Boolean): Unit = {
    new ForEachDStream(this, context.sparkContext.clean(foreachFunc, false), displayInnerRDDOps)
      .register()
  }

  /**
   * Return a new DStream in which each RDD is generated by applying a function on each RDD of
   * 'this' DStream.
   */
  ...
  def transformWith[U: ClassTag, V: ClassTag](
      other: DStream[U],
      transformFunc: (RDD[T], RDD[U]) => RDD[V]): DStream[V] = ssc.withScope {
    // because the DStream is reachable from the outer object here, and because
    // DStreams can't be serialized with closures, we can't proactively check
    // it for serializability and so we pass the optional false to SparkContext.clean
  ...
  def transformWith[U: ClassTag, V: ClassTag](
      other: DStream[U],
      transformFunc: (RDD[T], RDD[U], Time) => RDD[V]): DStream[V] = ssc.withScope {
    // because the DStream is reachable from the outer object here, and because
    // DStreams can't be serialized with closures, we can't proactively check
    // it for serializability and so we pass the optional false to SparkContext.clean
    val cleanedF = ssc.sparkContext.clean(transformFunc, false)
  ...
  /**
   * Return a new DStream in which each RDD contains all the elements in seen in a sliding window
   * of time over this DStream. The new DStream generates RDDs with the same interval as this
   * DStream.
  ...
  /**
   * Return a new DStream in which each RDD contains all the elements in seen in a sliding window
   * of time over this DStream.
   * @param windowDuration
   *   width of the window; must be a multiple of this DStream's batching interval
   * @param slideDuration
   *   sliding interval of the window (i.e., the interval after which the new DStream will
   *   generate RDDs); must be a multiple of this DStream's batching interval
   */
  ...
  def reduceByWindow(
      ...
      slideDuration: Duration): DStream[T] = ssc.withScope {
   ...
   *   2. "inverse reduce" the old values that left the window (e.g., subtracting old counts) This
   *      is more efficient than reduceByWindow without "inverse reduce" function. However, it is
   *      applicable to only "invertible reduce functions".
   * @param reduceFunc
   *   associative and commutative reduce function
   * @param invReduceFunc
   ...
   * of elements in a sliding window over this DStream. Hash partitioning is used to generate the
   * RDDs with Spark's default number of partitions.
   * @param windowDuration
   *   width of the window; must be a multiple of this DStream's batching interval
   * @param slideDuration
   *   sliding interval of the window (i.e., the interval after which the new DStream will
  ...
  /**
   * Return a new DStream in which each RDD contains the count of distinct elements in RDDs in a
   * sliding window over this DStream. Hash partitioning is used to generate the RDDs with
   * `numPartitions` partitions (Spark's default number of partitions if `numPartitions` not
   * specified).
   * @param windowDuration
   *   width of the window; must be a multiple of this DStream's batching interval
   * @param slideDuration
  ...
  def slice(fromTime: Time, toTime: Time): Seq[RDD[T]] = ssc.withScope {
    ...
    val alignedToTime = if ((toTime - zeroTime).isMultipleOf(slideDuration)) {
      ...
      logWarning(
        log"toTime (${MDC(TO_TIME, toTime)}) is not a multiple of slideDuration " +
    ...
    val alignedFromTime = if ((fromTime - zeroTime).isMultipleOf(slideDuration)) {
      ...
      logWarning(
        log"fromTime (${MDC(FROM_TIME, fromTime)}) is not a multiple of slideDuration " +
          log"(${MDC(SLIDE_DURATION, slideDuration)})")
      fromTime.floor(slideDuration, zeroTime)
    }

    logInfo(
      log"Slicing from ${MDC(LogKeys.FROM_TIME, fromTime)} to " +
  ...
  /**
   * Save each RDD in this DStream as a Sequence file of serialized objects. The file name at each
   * batch interval is generated based on `prefix` and `suffix`: "prefix-TIME_IN_MS.suffix".
   */
  ...
  /**
   * Save each RDD in this DStream as at text file, using string representation of elements. The
   * file name at each batch interval is generated based on `prefix` and `suffix`:
  ...
  /**
   * Register this streaming as an output stream. This would ensure that RDDs of this DStream will
   * be generated.
   */
