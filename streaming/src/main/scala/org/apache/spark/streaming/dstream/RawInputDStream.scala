...
/**
 * An input stream that reads blocks of serialized objects from a given network address. The
 * blocks will be inserted directly into the block store. This is the fastest way to get data into
 * Spark Streaming, though it requires the sender to batch data and serialize it in the format
 * that the system is configured with.
 */
...
private[streaming] class RawNetworkReceiver(host: String, port: Int, storageLevel: StorageLevel)
