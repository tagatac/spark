...
/**
 * This class uses Kafka's own [[Admin]] API to read data offsets from Kafka. The
 * [[ConsumerStrategy]] class defines which Kafka topics and partitions should be read by this
 * source. These strategies directly correspond to the different consumption options in. This
 * class is designed to return a configured [[Admin]] that is used by the [[KafkaSource]] to query
 * for the offsets. See the docs on [[org.apache.spark.sql.kafka010.ConsumerStrategy]] for more
 * details.
 *
...
private[kafka010] class KafkaOffsetReaderAdmin(
  ...
  /**
   * An AdminClient used in the driver to query the latest Kafka offsets. This only queries the
   * offsets because AdminClient has no functionality to commit offsets like KafkaConsumer.
   */
    ...
    Option(driverKafkaParams.get(ConsumerConfig.ISOLATION_LEVEL_CONFIG)) match {
      ...
      case _ =>
        throw new IllegalArgumentException(
          s"${ConsumerConfig.ISOLATION_LEVEL_CONFIG} " +
  ...
  private def listOffsets(admin: Admin, listOffsetsParams: ju.Map[TopicPartition, OffsetSpec]) = {
    admin
      .listOffsets(listOffsetsParams, listOffsetsOptions)
      .all()
      .get()
      .asScala
  ...
  override def fetchGlobalTimestampBasedOffsets(
    ...
    val fnRetrievePartitionOffsets: ju.Set[TopicPartition] => Map[TopicPartition, Long] = { tps =>
      val listOffsetsParams = tps.asScala
        .map { tp =>
          tp -> OffsetSpec.forTimestamp(timestamp)
        }
        .toMap
  ...
  private def partitionsAssignedToAdmin(
      body: ju.Set[TopicPartition] => Map[TopicPartition, Long]): Map[TopicPartition, Long] = {
