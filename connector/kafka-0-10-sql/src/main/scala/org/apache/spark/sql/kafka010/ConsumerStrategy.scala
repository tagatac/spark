...
/**
 * Subscribe allows you to subscribe to a fixed collection of topics. SubscribePattern allows you
 * to use a regex to specify topics of interest. Note that unlike the 0.8 integration, using
 * Subscribe or SubscribePattern should respond to adding partitions during a running stream.
 * Finally, Assign allows you to specify a fixed collection of partitions. All three strategies
 * have overloaded constructors that allow you to specify the starting offset for a particular
 * partition.
 */
private[kafka010] sealed trait ConsumerStrategy extends Logging {
  ...
  def createAdmin(kafkaParams: ju.Map[String, Object]): Admin = {
    ...
    logDebug(
      s"Admin params: ${KafkaRedactionUtil.redactParams(updatedKafkaParams.asScala.toSeq)}")
  ...
  protected def retrieveAllPartitions(admin: Admin, topics: Set[String]): Set[TopicPartition] = {
    admin
      ...
      .flatMap { case (topic, topicDescription) =>
