 ...
 *
 *   - The [[KafkaSourceOffset]] is the custom [[Offset]] defined for this source that contains a
 *     map of TopicPartition -> offset. Note that this offset is 1 + (available offset). For
 *     example if the last record in a Kafka topic "t", partition 2 is offset 5, then
 *     KafkaSourceOffset will contain TopicPartition("t", 2) -> 6. This is done keep it consistent
 *     with the semantics of `KafkaConsumer.position()`.
 *   - The [[KafkaSource]] written to do the following.
 *   - As soon as the source is created, the pre-configured [[KafkaOffsetReader]] is used to query
 *     the initial offsets that this source should start reading from. This is used to create the
 *     first batch.
 *   - `getOffset()` uses the [[KafkaOffsetReader]] to query the latest available offsets, which
 *     are returned as a [[KafkaSourceOffset]].
 *   - `getBatch()` returns a DF that reads from the 'start offset' until the 'end offset' in for
 *     each partition. The end offset is excluded to be consistent with the semantics of
 *     [[KafkaSourceOffset]] and `KafkaConsumer.position()`.
 *   - The DF returned is based on [[KafkaSourceRDD]] which is constructed such that the data from
 *     Kafka topic + partition is consistently read by the same executors across batches, and
 *     cached KafkaConsumers in the executors can be reused efficiently. See the docs on
 *     [[KafkaSourceRDD]] for more details.
 *
 * Zero data lost is not guaranteed when topics are deleted. If zero data lost is critical, the
 * user must make sure all messages in a topic have been processed when deleting a topic.
 *
 * There is a known issue caused by KAFKA-1894: the query using KafkaSource maybe cannot be
 * stopped. To avoid this issue, you should make sure stopping the query before stopping the Kafka
 * brokers and not use wrong broker addresses.
 */
private[kafka010] class KafkaSource(
  ...
  private val pollTimeoutMs =
    sourceOptions
      .getOrElse(CONSUMER_POLL_TIMEOUT, (sc.conf.get(NETWORK_TIMEOUT) * 1000L).toString)
      .toLong
  ...
  private[kafka010] val maxTriggerDelayMs =
    Utils.timeStringAsMs(
      sourceOptions.get(MAX_TRIGGER_DELAY).getOrElse(DEFAULT_MAX_TRIGGER_DELAY))
    ...
    val offsets = if (limits.exists(_.isInstanceOf[ReadAllAvailable])) {
      ...
      lowerLimit
        .flatMap { limit =>
          ...
          val skipBatch =
            delayBatch(limit.minRows, latest, currentOffsets.get, limit.maxTriggerDelayMs)
          if (skipBatch) {
            logDebug(
  ...
  /**
   * Returns the data that is between the offsets [`start.get.partitionToOffsets`,
   * `end.partitionToOffsets`), i.e. end.partitionToOffsets is exclusive.
   */
  override def getBatch(start: Option[Offset], end: Offset): DataFrame = {
    ...
    logInfo(
      log"GetBatch called with start = ${MDC(FROM_OFFSET, start)}, " +
    ...
    val rdd = if (includeHeaders) {
      new KafkaSourceRDD(sc, executorKafkaParams, offsetRanges, pollTimeoutMs, failOnDataLoss)
