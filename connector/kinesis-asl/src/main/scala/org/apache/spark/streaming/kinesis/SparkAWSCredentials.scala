...
private[kinesis] sealed trait SparkAWSCredentials extends Serializable {
  ...
  /**
   * Return an AwsCredentialProvider instance that can be used by the Kinesis Client Library to
   * authenticate to AWS services (Kinesis, CloudWatch and DynamoDB).
   */
 ...
 * DefaultCredentialsProvider if unable to construct a StaticCredentialsProvider instance with the
 * provided arguments (e.g. if they are null).
 */
private[kinesis] final case class BasicCredentials(awsAccessKeyId: String, awsSecretKey: String)
  ...
  def provider: AwsCredentialsProvider = try {
    ...
    case e: IllegalArgumentException =>
      logWarning(
        "Unable to construct StaticCredentialsProvider with provided keypair; " +
          "falling back to DefaultCredentialsProvider.",
        e)
...
/**
 * Returns an StsAssumeRoleCredentialsProvider instance which assumes an IAM role in order to
...
object SparkAWSCredentials {
  ...
  class Builder {
     ...
     *
     * @note
     *   The given AWS keypair will be saved in DStream checkpoints if checkpointing is enabled.
     *   Make sure that your checkpoint directory is secure. Prefer using the default credentials
     *   provider instead if possible
    ...
    def basicCredentials(accessKeyId: String, secretKey: String): Builder = {
      basicCreds = Option(
        BasicCredentials(awsAccessKeyId = accessKeyId, awsSecretKey = secretKey))
      this
    }

    /**
     * Use STS to assume an IAM role for temporary session-based authentication. Will use
     * configured long-lived credentials for authorizing to STS itself (either the default
     * provider chain or a configured keypair).
     *
     * @param roleArn
     ...
     * Use STS to assume an IAM role for temporary session-based authentication. Will use
     * configured long-lived credentials for authorizing to STS itself (either the default
     * provider chain or a configured keypair). STS will validate the provided external ID with
     * the one defined in the trust policy of the IAM role to be assumed (if one is present).
     *
     * @param roleArn
     *   ARN of IAM role to assume via STS
     * @param sessionName
     ...
     * parameters.
     *
     *   - The long-lived credentials will either be [[DefaultCredentials]] or
     *     [[BasicCredentials]] if they were provided.
     *   - If STS credentials were provided, the configured long-lived credentials will be added
     *     to them and the result will be returned.
  ...
  /**
   * Creates a [[SparkAWSCredentials.Builder]] for constructing [[SparkAWSCredentials]] instances.
   *
   * @since 2.2.0
   *
   * @return
   *   [[SparkAWSCredentials.Builder]] instance
   */
