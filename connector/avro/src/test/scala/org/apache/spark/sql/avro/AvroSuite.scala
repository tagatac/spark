...
abstract class AvroSuite
  ...
  private def checkUnionStableId(
    ...
    withTempDir { dir =>
      val unionType = Schema.createUnion(types.asJava)
      val fields =
  ...
  test("resolve avro data source") {
    ...
    Seq("org.apache.spark.sql.avro.AvroFileFormat", databricksAvro).foreach { provider =>
      assert(
        DataSource.lookupDataSource(provider, spark.sessionState.conf) ===
  ...
  test("reading and writing partitioned data") {
    ...
    for (field <- fields) {
      withTempPath { dir =>
        ...
        assert(
          input.select(field).collect().map(_.toSeq).toSet ===
      ...
      val avroSchema: Schema = {
        val union = Schema.createUnion(
          List(
            ...
            Schema.create(Type.NULL)).asJava)
        val fields = Seq(new Field("field1", union, "doc", null.asInstanceOf[AnyVal])).asJava
        val schema = Schema.createRecord("name", "docs", "namespace", false)
        schema.setFields(fields)
        schema
  ...
  test("SPARK-43333: Stable field names when converting Union type") {
    checkUnionStableId(
      ...
      Seq((42, Row(42, null)), ("Alice", Row(null, "Alice"))))
    ...
    checkUnionStableId(
      List(
        ...
        Schema.createRecord(
          ...
          false,
          List[Schema.Field](
            new Schema.Field("f", Schema.createFixed("myfield", "", null, 6))).asJava),
        Schema.createRecord(
          "myrecord2",
          "",
          null,
          false,
          List[Schema.Field](new Schema.Field("f", Schema.create(Type.FLOAT))).asJava)),
    ...
    // Somehow Avro allows named type "array", but doesn't allow an array type in the same union.
    {
      val e = intercept[Exception] {
        Schema.createUnion(
          List(
            Schema.createArray(Schema.create(Type.FLOAT)),
            Schema.createFixed("array", "", null, 6)).asJava)
      }
      assert(e.getMessage.contains("Duplicate in union"))
    }
    {
      val e = intercept[Exception] {
        Schema.createUnion(List(Schema.createFixed("long", "", null, 6)).asJava)
      }
    ...
    {
      val e = intercept[Exception] {
        Schema.createUnion(List(Schema.createFixed("bytes", "", null, 6)).asJava)
      }
      assert(e.getMessage.contains("Schemas may not be named after primitives"))
    }
  }
  ...
  test("Complex Union Type") {
    withTempPath { dir =>
      ...
      val complexUnionType = SchemaBuilder
        .unionOf()
        .intType()
        .and()
        .stringType()
  ...
  test("Date field type") {
    withTempPath { dir =>
      val schema = StructType(
        Seq(StructField("float", FloatType, true), StructField("date", DateType, true)))
    ...
    withTempPath { dir =>
      val testSchema = StructType(
        Seq(
          ...
          StructField(
            "struct_array",
            ArrayType(StructType(Seq(StructField("name", StringType, true)))))))
        ...
        spark.sparkContext.parallelize(
          Seq(
            Row(
              ...
              Array[Array[String]](
                Array[String]("CSH, tearing down the walls that divide us", "-jd")),
  ...
  test("support of various data types") {
    ...
    assert(
      array_of_boolean.map(_(0).asInstanceOf[scala.collection.Seq[Boolean]].size).toSet ==
        Set(3, 1, 0))
  ...
  test(
    "SPARK-43380: Fix Avro data type conversion" +
  ...
  Seq(
    ...
    "local-timestamp-micros").foreach { timeLogicalType =>
    test(s"converting $timeLogicalType type to long in avro") {
      withTempPath { path =>
        ...
        checkAnswer(spark.read.schema(s"dt long").format("avro").load(path.toString), Row(100L))
      }
    }
  }
  ...
  test("converting some specific sparkSQL types to avro") {
    withTempPath { tempDir =>
      ...
      val cityRDD = spark.sparkContext.parallelize(
        Seq(
  ...
  test("correctly read long as date/timestamp type") {
    withTempPath { tempDir =>
      ...
      val schema = StructType(
        Seq(StructField("_1", DateType, false), StructField("_2", TimestampType, false)))
  ...
  test("SPARK-34416: support user provided both avro schema and avro schema url") {
    ...
    val result = spark.read
      ...
      .collect()
    val expected =
      spark.read.format("avro").load(testAvro).select("union_int_long_null").collect()
    assert(result.sameElements(expected))
  }
    ...
    withTempPath { tempDir =>
      ...
      val e = intercept[SparkException] {
        spark
          .createDataFrame(
            spark.sparkContext.parallelize(
              Seq(Row("SPADES"), Row("NOT-IN-ENUM"), Row("HEARTS"), Row("DIAMONDS"))),
            StructType(Seq(StructField("Suit", StringType, true))))
          .write
          .format("avro")
          .option("avroSchema", avroSchema)
      ...
      assertExceptionMsg[IncompatibleSchemaException](
        e,
  ...
  test("support user provided avro schema for writing non-nullable enum type") {
    withTempPath { tempDir =>
      ...
      val dfWithNull = spark.createDataFrame(
        spark.sparkContext.parallelize(
          Seq(
            Row("SPADES"),
            Row(null),
      ...
      checkError(
        ...
        parameters = Map("name" -> "`Suit`", "dataType" -> expectedDatatype))
  ...
  test("to_avro nested struct schema nullability mismatch") {
    Seq((true, false), (false, true)).foreach { case (innerNull, outerNull) =>
      ...
      checkError(
        ...
        condition = "AVRO_CANNOT_WRITE_NULL_FIELD",
        parameters = Map("name" -> expectedErrorName, "dataType" -> expectedErrorSchema))
    }
  }
  ...
  test("to_avro with reordered fields and nullable target succeeds") {
    ...
    val df = Seq(("B", null.asInstanceOf[String]))
      .toDF("b", "a")
  ...
  test("to_avro with reordered fields fails with correct field name") {
    ...
    val df = Seq((null.asInstanceOf[String], "A"))
      .toDF("b", "a")
      .select(struct($"b", $"a").as("s"))
    ...
    checkError(
      ...
      condition = "AVRO_CANNOT_WRITE_NULL_FIELD",
      parameters = Map("name" -> "`b`", "dataType" -> "\"string\""))
  }
  ...
  test("support user provided avro schema for writing nullable fixed type") {
    withTempPath { tempDir =>
      ...
      val df = spark.createDataFrame(
        spark.sparkContext.parallelize(Seq(Row(Array(192, 168).map(_.toByte)), Row(null))),
      ...
      val e1 = intercept[SparkException] {
        spark
          .createDataFrame(
            spark.sparkContext.parallelize(Seq(Row(Array(192, 168, 1).map(_.toByte)))),
            StructType(Seq(StructField("fixed2", BinaryType, true))))
          .write
          .format("avro")
          .option("avroSchema", avroSchema)
      ...
      val e2 = intercept[SparkException] {
        spark
          .createDataFrame(
            spark.sparkContext.parallelize(Seq(Row(Array(192).map(_.toByte)))),
            StructType(Seq(StructField("fixed2", BinaryType, true))))
          .write
          .format("avro")
          .option("avroSchema", avroSchema)
  ...
  test("support user provided avro schema for writing non-nullable fixed type") {
    withTempPath { tempDir =>
      ...
      val df = spark.createDataFrame(
        spark.sparkContext.parallelize(
          Seq(Row(Array(192, 168).map(_.toByte)), Row(Array(1, 1).map(_.toByte)))),
  ...
  test("support user provided avro schema for writing / reading fields with different ordering") {
    withTempPath { tempDir =>
      ...
      val df = spark.createDataFrame(
        ...
        StructType(
          Seq(StructField("Age", IntegerType, false), StructField("Name", StringType, false))))
  ...
  test(
    "support user provided non-nullable avro schema " +
      "for nullable catalyst schema without any null record") {
    withTempPath { tempDir =>
      val catalystSchema =
        StructType(
          Seq(StructField("Age", IntegerType, true), StructField("Name", StringType, true)))
      ...
      val ex = intercept[SparkException] {
        spark
          .createDataFrame(spark.sparkContext.parallelize(Seq(Row(2, null))), catalystSchema)
          .write
          .format("avro")
          .option("avroSchema", avroSchema)
          .save(s"$tempDir/${UUID.randomUUID()}")
      ...
      checkError(
        ...
        condition = "AVRO_CANNOT_WRITE_NULL_FIELD",
        parameters = Map("name" -> "`Name`", "dataType" -> "\"string\""))
    }
  }

  test(
    "support user provided nullable avro schema " +
    ...
    val df =
      spark.createDataFrame(spark.sparkContext.parallelize(Seq(Row(2, "Aurora"))), catalystSchema)
  ...
  test("SPARK-34365: support reading renamed schema using positionalFieldMatching") {
    ...
    assert(
      dfLoaded.select($"foo".as("string"), $"foo_map".as("simple_map")).collect().toSet ===
  ...
  test("SPARK-34365: support writing with renamed schema using positionalFieldMatching") {
    withTempDir { tempDir =>
      val avroSchema = SchemaBuilder
        ...
        .requiredString("foo")
        .name("foo_map")
        .`type`(Schema.createMap(Schema.create(Schema.Type.INT)))
        .noDefault()
        .endRecord()
      val expectedDf = spark.read.format("avro").load(testAvro).select("string", "simple_map")
      ...
      assert(
        reloadedDf.select($"foo".as("string"), $"foo_map".as("simple_map")).collect().toSet ===
          expectedDf.collect().toSet)
    }
  }
  ...
  test("non-matching complex union types") {
    val catalystSchema = new StructType().add(
      "Union",
      new StructType()
    ...
    val df = spark.createDataFrame(
      spark.sparkContext.parallelize(Seq(Row(Row(1, null)))),
      catalystSchema)
    ...
    for ((unionTypes, compatible) <- Seq(
        ...
        (
          Seq(nullS, SchemaBuilder.record("r").fields().requiredString("f2").endRecord()),
          false))) {
      val avroSchema = SchemaBuilder
        .record("test_schema")
        .fields()
        .name("union")
        .`type`(Schema.createUnion(unionTypes: _*))
        .noDefault()
        .endRecord()
        .toString()
  ...
  test("read avro with user defined schema: read partial columns") {
    val partialColumns = StructType(
      Seq(
        ...
        StructField(
          "record",
          StructType(Seq(StructField("value_field", StringType, false))),
          false),
  ...
  test("Validate namespace in avro file that has nested records with the same name") {
    withTempPath { dir =>
      val writeDf =
        spark.createDataFrame(List(NestedTop(1, NestedMiddle(2, NestedBottom(3, "1")))))
    ...
    val sparkSchema = StructType(
      Seq(
        ...
        StructField(
          ...
          nullable = false)))
    val employeeType =
      SchemaConverters.toAvroType(sparkSchema, recordName = "employee", nameSpace = "foo.bar")
        ...
        StructField(
          ...
          StructType(
            Seq(
              ...
              StructField("state", StringType, nullable = false))),
          nullable = false)))
    val employeeType = SchemaConverters.toAvroType(sparkSchema, recordName = "employee")
    ...
    withTempPath { tempDir =>
      ...
      val writeDf = spark.createDataFrame(
        List(
          NestedTopArray(
            1,
            NestedMiddleArray(2, Array(NestedBottom(3, "1"), NestedBottom(4, "2"))))))
    ...
    withTempPath { tempDir =>
      ...
      val writeDf = spark.createDataFrame(
        List(
          NestedTopMap(
            1,
            NestedMiddleMap(2, Map("1" -> NestedBottom(3, "1"), "2" -> NestedBottom(4, "2"))))))
  ...
  test("SPARK-24805: do not ignore files without .avro extension by default") {
    withTempDir { dir =>
      Files.copy(Paths.get(new URI(episodesAvro)), Paths.get(dir.getCanonicalPath, "episodes"))
  ...
  test("SPARK-24881: write with compression - avro options") {
    def getCodec(dir: String): Option[String] = {
      ...
      files
        ...
        .map(v => if (v == "null") UNCOMPRESSED.lowerCaseName() else v)
        .headOption
    }
  ...
  private def checkSchemaWithRecursiveLoop(
      avroSchema: String,
      recursiveFieldMaxDepth: Int): Unit = {
  ...
  private def checkSparkSchemaEquals(
    ...
    val sparkSchema =
      SchemaConverters
        .toSqlType(new Schema.Parser().parse(avroSchema), false, "", recursiveFieldMaxDepth)
        .dataType
  ...
  test("Translate recursive schema - union - 2 non-null fields") {
    val avroSchema = """
     ...
     |}
    """.stripMargin
    val nonRecursiveFields = new StructType()
    ...
    for (i <- 1 to 5) {
      checkSparkSchemaEquals(avroSchema, expectedSchema, i)
      expectedSchema =
        nonRecursiveFields.add("children", new ArrayType(expectedSchema, false), nullable = true)
    }
  }
  ...
  test("Translate recursive schema - map") {
    ...
    for (i <- 1 to 5) {
      checkSparkSchemaEquals(avroSchema, expectedSchema, i)
      expectedSchema = nonRecursiveFields.add(
        "map",
        new MapType(StringType, expectedSchema, false),
        nullable = false)
    }
  }
            ...
            StructType(
              Seq(
                ...
                StructField(
                  "Name",
                  StructType(
                    Seq(StructField("Id", IntegerType), StructField("Name", NullType)))))))))
  ...
  ignore("SPARK-31855: generate test files for checking compatibility with Spark 2.4/3.2") {
    ...
    withDefaultTimeZone(LA) {
      withSQLConf(SQLConf.SESSION_LOCAL_TIMEZONE.key -> LA.getId) {
        save(Seq("1001-01-01"), "date", s"before_1582_date_v$version.avro")
        save(
  ...
  private def runInMode(modes: Seq[LegacyBehaviorPolicy.Value])(
  ...
  test("SPARK-31183, SPARK-37705: rebasing milliseconds timestamps in write") {
    ...
    DateTimeTestUtils.withDefaultTimeZone(DateTimeTestUtils.JST) {
      withSQLConf(SQLConf.SESSION_LOCAL_TIMEZONE.key -> DateTimeTestUtils.LA.getId) {
        ...
        Seq("""{"type": "long","logicalType": "timestamp-millis"}""", """"long"""").foreach {
          tsType =>
  ...
  test(
    "SPARK-33163, SPARK-37705: write the metadata key 'org.apache.spark.legacyDateTime' " +
      "and 'org.apache.spark.timeZone'") {
    def saveTs(dir: java.io.File): Unit = {
      Seq(Timestamp.valueOf("2020-10-15 01:02:03"))
        .toDF()
        .repartition(1)
        .write
        .format("avro")
  ...
  test("SPARK-35427: datetime rebasing in the EXCEPTION mode") {
    ...
    withSQLConf(SQLConf.AVRO_REBASE_MODE_IN_READ.key -> EXCEPTION.toString) {
      Seq(
        ...
        "before_1582_timestamp_millis_v2_4_5.avro").foreach { fileName =>
      ...
      withView("v") {
        ...
        withTempDir { dir =>
          checkError(
            ...
            parameters =
              Map("datasource" -> "AvroFileFormat", "columnName" -> "`(IF((ID = 1), 1, 0))`"))
        }
  ...
  test("SPARK-53973: boolean Avro options reject non-boolean values") {
    Seq(
      ...
      AvroOptions.IGNORE_EXTENSION).foreach { opt =>
      ...
      checkError(
        ...
        parameters =
          Map("optionName" -> opt, "message" -> "Cannot cast value 'not_bool' to Boolean."))
    }
  }
  ...
  test("SPARK-36271: V1 insert should check schema field name too") {
    withView("v") {
      ...
      withTempDir { dir =>
        checkError(
          exception = intercept[AnalysisException] {
            sql("SELECT ID, IF(ID=1,1,0) FROM v").write
      ...
      withTempDir { dir =>
        checkError(
          exception = intercept[AnalysisException] {
            sql("SELECT NAMED_STRUCT('(IF((ID = 1), 1, 0))', IF(ID=1,ID,0)) AS col1 FROM v").write
              .mode(SaveMode.Overwrite)
              .format("avro")
              .save(dir.getCanonicalPath)
          },
...
class AvroV2Suite extends AvroSuite with ExplainSuiteHelper {
  ...
  test("Avro source v2: support partition pruning") {
    withTempPath { dir =>
      ...
      val filterCondition = df.queryExecution.optimizedPlan.collectFirst { case f: Filter =>
        f.condition
      }
  ...
  test(
    "Avro source v2: same result with different orders of data filters and partition filters") {
    withTempPath { path =>
  ...
  test("explain formatted on an avro data source v2") {
    withTempDir { dir =>
      ...
      assert(
        expected_plan_fragment.r.findAllMatchIn(normalizedOutput).length == 1,
        normalizedOutput)
    }
  }
  ...
  test("TIME type read/write with Avro format") {
    withTempPath { dir =>
      ...
      assert(
        row.getAs[java.time.LocalTime]("midnight") ==
          java.time.LocalTime.of(0, 0, 0, 123456000))
      assert(
        row.getAs[java.time.LocalTime]("noon") ==
          java.time.LocalTime.of(12, 34, 56, 789012000))
      assert(
        row.getAs[java.time.LocalTime]("max_time") ==
  ...
  test("TIME type precision metadata is preserved in Avro") {
    withTempPath { dir =>
      ...
      (0 to 6).foreach { p =>
        assert(
          readDf.schema(s"time_p$p").dataType == TimeType(p),
    ...
    withTempDir { dir =>
      ...
      geoTestCases.foreach { case (expr, expectedType) =>
        checkError(
          ...
          condition = "UNSUPPORTED_DATA_TYPE_FOR_DATASOURCE",
          parameters =
            Map("columnName" -> "`g`", "columnType" -> expectedType, "format" -> "Avro"))
      }
    }
  }
}
