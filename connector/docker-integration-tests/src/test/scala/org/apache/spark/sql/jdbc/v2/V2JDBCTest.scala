...
private[v2] trait V2JDBCTest
  ...
  /**
   * Returns a copy of the given [[StructType]] with the specified metadata key removed from all
   * of its fields.
   */
  ...
  def testUpdateColumnNullability(tbl: String): Unit = {
    ...
    expectedSchemaWithoutJdbcClientType =
      removeMetadataFromAllFields(expectedSchema, "jdbcClientType")
    schemaWithoutJdbcClientType = removeMetadataFromAllFields(t.schema, "jdbcClientType")
    assert(schemaWithoutJdbcClientType === expectedSchemaWithoutJdbcClientType)
  ...
  private def checkErrorFailedJDBC(e: AnalysisException, condition: String, tbl: String): Unit = {
    checkErrorMatchPVals(
      exception = e,
      condition = condition,
      parameters = Map("url" -> "jdbc:.*", "tableName" -> s"`$tbl`"))
  }
    ...
    tableNameToPartinioningOptions.get(testTableName) match {
      case Some(partitionInfo) =>
        val partitioningInfoString = Seq(
          ...
          s"'partitionColumn' '${partitionInfo.partitionColumn}'").mkString(",")
  ...
  def multiplePartitionAdditionalCheck(
    ...
    assert(
      jdbcRdd.isDefined && jdbcRdd.get.getNumPartitions ==
  ...
  test("SPARK-33034: ALTER TABLE ... rename column") {
    withTable(s"$catalogName.alt_table") {
      sql(
        s"CREATE TABLE $catalogName.alt_table (ID STRING NOT NULL," +
  ...
  test("SPARK-36895: Test INDEX Using SQL") {
    if (supportsIndex) {
      withTable(s"$catalogName.new_table") {
        sql(
          s"CREATE TABLE $catalogName.new_table(col1 INT, col2 INT, col3 INT," +
            " col4 INT, col5 INT)")
        ...
        assert(
          m.contains(s"Index Type $indexType is not supported." +
        ...
        sql(
          s"CREATE index i2 ON $catalogName.new_table (col2, col3, col5)" +
  ...
  test("SPARK-48172: Test CONTAINS") {
    ...
    val df2 =
      spark.sql(s"""SELECT * FROM $catalogAndNamespace.${caseConvert("pattern_testing_table")}
  ...
  gridTest("SPARK-37038: Test TABLESAMPLE")(partitioningEnabledTestCase) { partitioningEnabled =>
    if (supportsTableSample) {
      withTable(s"$catalogName.new_table") {
        ...
        val df2 = sql(
          s"SELECT * FROM $catalogName.new_table $tableOptions " +
  ...
  gridTest("simple scan")(partitioningEnabledTestCase) { partitioningEnabled =>
    ...
    val df = sql(
      s"SELECT name, salary, bonus FROM $catalogAndNamespace." +
  ...
  gridTest("simple scan with LIMIT and OFFSET")(partitioningEnabledTestCase) {
    partitioningEnabled =>
      val (tableOptions, partitionInfo) = getTableOptions("employee", partitioningEnabled)
      val df = sql(
        s"SELECT name, salary, bonus FROM $catalogAndNamespace." +
  ...
  Seq(true, false).foreach { isDistinct =>
    ...
    gridTest(s"scan with aggregate push-down: VAR_POP $withOrWithout DISTINCT")(
      partitioningEnabledTestCase) { partitioningEnabled =>
      val (tableOptions, partitionInfo) = getTableOptions("employee", partitioningEnabled)
      val df = sql(
        s"SELECT VAR_POP(${distinct}bonus) FROM $catalogAndNamespace." +
          s"${caseConvert("employee")} $tableOptions " +
    ...
    gridTest(s"scan with aggregate push-down: VAR_SAMP $withOrWithout DISTINCT")(
      partitioningEnabledTestCase) { partitioningEnabled =>
      val (tableOptions, partitionInfo) = getTableOptions("employee", partitioningEnabled)
      val df = sql(
    ...
    gridTest(s"scan with aggregate push-down: STDDEV_POP $withOrWithout DISTINCT")(
      partitioningEnabledTestCase) { partitioningEnabled =>
      val (tableOptions, partitionInfo) = getTableOptions("employee", partitioningEnabled)
      val df = sql(
    ...
    gridTest(s"scan with aggregate push-down: STDDEV_SAMP $withOrWithout DISTINCT")(
      partitioningEnabledTestCase) { partitioningEnabled =>
      val (tableOptions, partitionInfo) = getTableOptions("employee", partitioningEnabled)
      val df = sql(
    ...
    gridTest(s"scan with aggregate push-down: COVAR_POP $withOrWithout DISTINCT")(
      partitioningEnabledTestCase) { partitioningEnabled =>
      val (tableOptions, partitionInfo) = getTableOptions("employee", partitioningEnabled)
      val df = sql(
    ...
    gridTest(s"scan with aggregate push-down: COVAR_SAMP $withOrWithout DISTINCT")(
      partitioningEnabledTestCase) { partitioningEnabled =>
      val (tableOptions, partitionInfo) = getTableOptions("employee", partitioningEnabled)
      val df = sql(
    ...
    gridTest(s"scan with aggregate push-down: CORR $withOrWithout DISTINCT")(
      partitioningEnabledTestCase) { partitioningEnabled =>
      val (tableOptions, partitionInfo) = getTableOptions("employee", partitioningEnabled)
      val df = sql(
    ...
    gridTest(s"scan with aggregate push-down: REGR_INTERCEPT $withOrWithout DISTINCT")(
      partitioningEnabledTestCase) { partitioningEnabled =>
      val (tableOptions, partitionInfo) = getTableOptions("employee", partitioningEnabled)
      val df = sql(
    ...
    gridTest(s"scan with aggregate push-down: REGR_SLOPE $withOrWithout DISTINCT")(
      partitioningEnabledTestCase) { partitioningEnabled =>
      val (tableOptions, partitionInfo) = getTableOptions("employee", partitioningEnabled)
      val df = sql(
    ...
    gridTest(s"scan with aggregate push-down: REGR_R2 $withOrWithout DISTINCT")(
      partitioningEnabledTestCase) { partitioningEnabled =>
      val (tableOptions, partitionInfo) = getTableOptions("employee", partitioningEnabled)
      val df = sql(
    ...
    gridTest(s"scan with aggregate push-down: REGR_SXY $withOrWithout DISTINCT")(
      partitioningEnabledTestCase) { partitioningEnabled =>
      val (tableOptions, partitionInfo) = getTableOptions("employee", partitioningEnabled)
      val df = sql(
  ...
  test("SPARK-52262: FAILED_JDBC.TABLE_EXISTS not thrown on connection error") {
    ...
    val invalidUrl = originalUrl
      .replace("localhost", "nonexistenthost")
