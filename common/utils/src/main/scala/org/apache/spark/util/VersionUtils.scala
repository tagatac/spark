...
private[spark] object VersionUtils {
  ...
  /**
   * Given a Spark version string, return the major version number. E.g., for 2.0.1-SNAPSHOT,
   * return 2.
   */
  def majorVersion(sparkVersion: String): Int = majorMinorVersion(sparkVersion)._1

  /**
   * Given a Spark version string, return the minor version number. E.g., for 2.0.1-SNAPSHOT,
   * return 0.
   */
  def minorVersion(sparkVersion: String): Int = majorMinorVersion(sparkVersion)._2

  /**
   * Given a Spark version string, return the short version string. E.g., for 3.0.0-SNAPSHOT,
   * return '3.0.0'.
   */
  ...
  /**
   * Given a Spark version string, return the (major version number, minor version number). E.g.,
   * for 2.0.1-SNAPSHOT, return (2, 0).
   */
  ...
  /**
   * Extracts the major, minor and patch parts from the input `version`. Note that if minor or
   * patch version is missing from the input, this will return 0 for these parts. Returns `None`
   * if the input is not of a valid format.
   *
   ...
   *
   * @return
   *   A non-empty option containing a 3-value tuple (major, minor, patch) iff the input is a
   *   valid version. `None` otherwise.
   */
