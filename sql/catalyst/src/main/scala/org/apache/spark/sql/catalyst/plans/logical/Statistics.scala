 ...
 *
 * '''NOTE''': concrete and/or overridden versions of statistics fields should pay attention to
 * the performance of the implementations. The reason is that estimations might get triggered in
 ...
 *
 * @param sizeInBytes
 *   Physical size in bytes. For leaf operators this defaults to 1, otherwise it defaults to the
 ...
 * @param isRuntime
 *   Whether the statistics is inferred from query stage runtime statistics during adaptive query
 *   execution.
 */
 ...
 *      Catalyst data type. For example, the internal type of DateType is Int, and that the
 *      internal type of TimestampType is Long.
...
case class ColumnStat(
  ...
  def updateCountStats(
    ...
    val newDistinctCount = EstimationUtils.updateStat(
      oldNumRows,
      newNumRows,
      distinctCount,
      updatedColumnStat.distinctCount)
    val newNullCount =
      EstimationUtils.updateStat(oldNumRows, newNumRows, nullCount, updatedColumnStat.nullCount)
    updatedColumnStat.copy(distinctCount = newDistinctCount, nullCount = newNullCount)
  }
}

/**
 * This class is an implementation of equi-height histogram. Equi-height histogram represents the
 * distribution of a column's values by a sequence of bins. Each bin has a value range and
...
object HistogramSerializer {
   ...
   * etc, we don't provide readability for their serialized formats in metastore (string-to-string
   * table properties). This is because it's hard or unnatural for these statistics to be human
   * readable. For example, a histogram usually cannot fit in a single, self-described property.
   * And for count-min-sketch, it's essentially unnatural to make it a readable string.
   */
