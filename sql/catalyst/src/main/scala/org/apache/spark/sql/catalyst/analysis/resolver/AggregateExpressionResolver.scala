...
class AggregateExpressionResolver(
   ...
   *   - Validation:
   *     1. [[ListAgg]] is not allowed in DISTINCT aggregates if it contains [[SortOrder]]
   *        different from its child;
   ...
   *   - In case we have an [[AggregateExpression]] inside a [[Sort]] operator, we need to handle
   *     it in a special way (see [[handleAggregateExpressionOutsideAggregate]] for more details).
  ...
  private def extractOuterAggregateExpression(
    ...
    val outerAggregateExpressionAlias = autoGeneratedAliasProvider.newOuterAlias(child =
