...
class CollationSuite extends DatasourceV2SQLBase with AdaptiveSparkPlanHelper {
  ...
  private val allFileBasedDataSources =
    collationPreservingSources ++ collationNonPreservingSources
  ...
  test("collate returns proper type") {
    Seq(
      ...
      "unicode_rtrim_ci",
      "utf8_lcase_rtrim",
      "utf8_binary_rtrim").foreach { collationName =>
  ...
  test("collate function syntax") {
    assert(
      sql(s"select collate('aaa', 'utf8_binary')").schema(0).dataType ==
        StringType("UTF8_BINARY"))
    assert(
      sql(s"select collate('aaa', 'utf8_binary_rtrim')").schema(0).dataType ==
        StringType("UTF8_BINARY_RTRIM"))
    assert(
      sql(s"select collate('aaa', 'utf8_lcase')").schema(0).dataType ==
        StringType("UTF8_LCASE"))
    assert(
      sql(s"select collate('aaa', 'utf8_lcase_rtrim')").schema(0).dataType ==
  ...
  test("collate function invalid input data type") {
    checkError(
      exception =
        intercept[ExtendedAnalysisException] { sql(s"select collate(1, 'UTF8_BINARY')") },
  ...
  test("equality check respects collation") {
    Seq(
      ...
      ("unicode_CI_rtrim", "aaa ", "AAA  ", true),
      ("unicode_CI_rtrim", "aaa", "bbb", false)).foreach {
  ...
  test("comparisons respect collation") {
    Seq(
      ...
      ("unicode_CI_rtrim", "aaa ", "aaa", false)).foreach {
      case (collationName, left, right, expected) =>
        checkAnswer(
          sql(s"select '$left' collate $collationName < '$right' collate $collationName"),
          Row(expected))
  ...
  test("aggregates count respects collation") {
    Seq(
      ...
      ("unicode_CI_rtrim", Seq("aaa", "AAA "), Seq(Row(2, "aaa")))).foreach {
  ...
  test("add collated column with alter table") {
    ...
    withTable(tableName) {
      ...
      checkAnswer(
        sql(s"SELECT DISTINCT COLLATION(c2) FROM $tableName"),
        Seq(Row(fullyQualifiedPrefix + collationName)))
  ...
  test("SPARK-47210: Implicit casting of collated strings") {
    ...
    withTable(tableName) {
      ...
      checkAnswer(sql(s"SELECT c1 FROM $tableName WHERE c1 = 'a'"), Seq(Row("a"), Row("A")))
      checkAnswer(sql(s"SELECT c1 FROM $tableName WHERE 'a' = c1"), Seq(Row("a"), Row("A")))

      // collate c1 to UTF8_BINARY because it is explicitly set
      checkAnswer(
        sql(s"SELECT c1 FROM $tableName WHERE c1 = COLLATE('a', 'UTF8_BINARY')"),
        Seq(Row("a")))

      // explicit collation propagates up
      checkAnswer(
        sql(
          s"SELECT c1 FROM $tableName " +
            s"WHERE c1 = SUBSTR(COLLATE('a', 'UNICODE'), 0)"),
        Row("a"))

      // in operator
      checkAnswer(sql(s"SELECT c1 FROM $tableName WHERE c1 IN ('a')"), Seq(Row("a"), Row("A")))
      // explicitly set collation inside IN operator
      checkAnswer(
        sql(s"SELECT c1 FROM $tableName WHERE c1 IN ('b', COLLATE('a', 'UTF8_BINARY'))"),
        Seq(Row("a")))

      // concat without type mismatch
      checkAnswer(
        sql(s"SELECT c1 FROM $tableName WHERE c1 || 'a' || 'a' = 'aaa'"),
        Seq(Row("a"), Row("A")))
      checkAnswer(
        sql(s"SELECT c1 FROM $tableName WHERE c1 || COLLATE(c2, 'UTF8_BINARY') = 'aa'"),
        Seq(Row("a")))

      // concat + in
      checkAnswer(
        sql(
          s"SELECT c1 FROM $tableName where c1 || 'a' " +
            s"IN (COLLATE('aa', 'UTF8_LCASE'))"),
        Seq(Row("a"), Row("A")))
      checkAnswer(
        sql(
          s"SELECT c1 FROM $tableName where (c1 || 'a') " +
            s"IN (COLLATE('aa', 'UTF8_BINARY'))"),
        Seq(Row("a")))
      checkAnswer(
        sql(
          s"SELECT c1 FROM $tableName where c1 || 'a' " +
            s"IN (COLLATE('aa', 'UTF8_LCASE_RTRIM'))"),
        Seq(Row("a"), Row("A")))
      checkAnswer(
        sql(
          s"SELECT c1 FROM $tableName where (c1 || 'a') " +
            s"IN (COLLATE('aa', 'UTF8_BINARY_RTRIM'))"),
        Seq(Row("a")))
      ...
      checkError(
        exception = intercept[AnalysisException] {
          sql(s"""
               ...
               |WHERE COLLATE('a', 'UTF8_BINARY') = COLLATE('a', 'UNICODE')""".stripMargin)
        },
        condition = "COLLATION_MISMATCH.EXPLICIT",
        parameters =
          Map("explicitTypes" -> """"STRING COLLATE UTF8_BINARY", "STRING COLLATE UNICODE""""))

      // in operator has different collations
      checkError(
        exception = intercept[AnalysisException] {
          sql(
            s"SELECT c1 FROM $tableName WHERE c1 IN " +
      ...
      checkError(
        exception = intercept[AnalysisException] {
          sql(
            s"SELECT c1 FROM $tableName WHERE COLLATE(c1, 'UNICODE') IN " +
              "(COLLATE('a', 'UTF8_BINARY'))")
        },
        condition = "COLLATION_MISMATCH.EXPLICIT",
        parameters =
          Map("explicitTypes" -> """"STRING COLLATE UNICODE", "STRING COLLATE UTF8_BINARY""""))

      // concat + in
      checkError(
        exception = intercept[AnalysisException] {
          sql(
            s"SELECT c1 FROM $tableName where c1 || COLLATE('a', 'UTF8_BINARY') IN " +
      ...
      checkAnswer(
        sql(s"SELECT typeof(array('a' COLLATE UNICODE, 'b')[1])"),
      ...
      checkError(
        ...
        condition = "COLLATION_MISMATCH.EXPLICIT",
        parameters =
          Map("explicitTypes" -> """"STRING COLLATE UNICODE", "STRING COLLATE UNICODE_CI""""))
  ...
  test("SPARK-49138: ArrayAppend and CreateMap coercion testing") {
    ...
    assert(
      df_create_map.schema.head.dataType ==
  ...
  test("SPARK-47692: Parameter marker with EXECUTE IMMEDIATE implicit casting") {
    ...
    checkAnswer(
      sql("""EXECUTE IMMEDIATE stmtStr1 USING
          | 'a' AS var1,
          | 'b' AS var2;""".stripMargin),
      Seq(Row(fullyQualifiedPrefix + "UTF8_BINARY")))

    checkAnswer(
      sql("""EXECUTE IMMEDIATE stmtStr2 USING
  ...
  test("shuffle respects collation") {
    ...
    val schema = StructType(
      StructField("col", StringType(CollationFactory.collationNameToId("UTF8_LCASE"))) :: Nil)
  ...
  test("Create dataframe with non utf8 binary collation") {
    ...
    checkAnswer(df.groupBy("name").count(), Seq(Row("Alice", 1), Row("Bob", 2)))
  }
    ...
    withTable(table) {
      ...
      checkError(
        ...
        parameters = Map(
          "colName" -> "`m`",
          "dataType" -> toSQLType(
            MapType(StringType(CollationFactory.collationNameToId("UTF8_LCASE")), StringType))),
  ...
  test("Joins on complex types containing collated strings") {
    ...
    withTable(tableLeft, tableRight) {
      ...
      Seq((tableLeft, "array('aaa')"), (tableRight, "array('AAA')")).map { case (tab, data) =>
        sql(s"insert into $tab values ($data)")
      }
      checkAnswer(
        sql(s"""
    ...
    withTable(tableLeft, tableRight) {
      ...
      Seq((tableLeft, "named_struct('fld', 'aaa')"), (tableRight, "named_struct('fld', 'AAA')"))
        .map { case (tab, data) =>
          sql(s"insert into $tab values ($data)")
        }
      checkAnswer(
  ...
  test("Check order by on table with collated string column") {
    ...
    Seq(
      ...
      (
        "UNICODE_CI",
        Seq((2, "bbb"), (1, "a"), (1, "A"), (1, "aaaa"), (3, "cc"), (2, "BbB")),
        Seq(1, 1, 1, 2, 2, 3))).foreach { case (collationName, data, expResult) =>
    ...
    Seq(
      ...
      (
        ...
        Seq(
          ...
          (1, "a", "a"),
          (5, "b", "d")),
        Seq(1, 2, 2, 3, 4, 5))).foreach { case (collationName, data, expResult) =>
      val collationSetup = if (collationName.isEmpty) "" else "collate " + collationName
      val tableName = "t"
      withTable(tableName) {
        sql(
          s"create table $tableName (c1 integer, c2 struct<" +
            s"s1: string $collationSetup," +
            s"s2: string $collationSetup>)")
        data.foreach { case (c1, s1, s2) =>
  ...
  test("Check order by on StructType with few collated fields") {
    ...
    withTable(tableName) {
      ...
      data.foreach { case (order, s1, s2, s3, s4, s5) =>
    ...
    Seq(
      ...
      (
        ...
        Seq(
          ...
          (5, Seq("b", "e")),
          (3, Seq("b"))),
        Seq(1, 2, 3, 4, 4, 5))).foreach { case (collationName, dataWithOrder, expResult) =>
      val collationSetup = if (collationName.isEmpty) "" else "collate " + collationName
      val tableName1 = "t1"
      val tableName2 = "t2"
      withTable(tableName1, tableName2) {
        ...
        sql(
          s"create table $tableName2 (c1 integer," +
            s" c2 struct<f1: array<string $collationSetup>>)")
        dataWithOrder.foreach { case (order, data) =>
  ...
  test("Check order by on StructType with different types containing collated strings") {
    ...
    withTable(tableName) {
      ...
      data.foreach { case (c1, (c2, c3, c4)) =>
        val c3String = c3
          .map { case (f1, f2, f3) => s"struct('$f1', '$f2', '$f3')" }
          .mkString(", ")
  ...
  test("Window functions with implicit collation rule") {
    ...
    withTable(table) {
      ...
      sql(
        s"INSERT INTO $table VALUES ('aA', 'aA', 2), ('Aa', 'Aa', 1)," +
          s"('ab', 'ab', 3), ('aa', 'aa', 1)")

      checkAnswer(
        sql(
          s"select s1 collate unicode = " +
  ...
  test("CommonExpressionRef inherits different collation strengths") {
    ...
    withTable(table) {
      ...
      checkAnswer(sql(s"SELECT NULLIF(c1, 'TEST') FROM $table"), Seq(Row(null), Row("OTHER")))
    }
    ...
    withTable(table) {
      ...
      sql(s"INSERT INTO $table VALUES ('TEST'), ('OTHER')")
      checkAnswer(sql(s"SELECT NULLIF('test', c1) FROM $table"), Seq(Row(null), Row("test")))
    }
  }
  ...
  test("hash join should be used for collated strings if sort merge join is not forced") {
    ...
    val testCases = Seq(
      ...
      HashJoinTestCase(
        ...
        "aa",
        "AA",
        Seq(Row("aa", 1, "AA", 2), Row("aa", 1, "aa", 2))),
        ...
        withSQLConf(SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> broadcastJoinThreshold.toString) {
          ...
          if (isSortMergeForced) {
            ...
            if (!CollationFactory.fetchCollation(t.collation).supportsBinaryEquality) {
              assert(collectFirst(queryPlan) { case b: HashJoin =>
                b.leftKeys.head
  ...
  test(
    "hash join should be used for arrays of collated strings if sort merge join is not forced") {
    val t1 = "T_1"
    val t2 = "T_2"

    case class HashJoinTestCase[R](collation: String, data1: String, data2: String, result: R)
    val testCases = Seq(
      HashJoinTestCase("UTF8_BINARY", "aa", "AA", Seq(Row(Seq("aa"), 1, Seq("aa"), 2))),
      HashJoinTestCase(
        "UTF8_LCASE",
        "aa",
        "AA",
        Seq(Row(Seq("aa"), 1, Seq("AA"), 2), Row(Seq("aa"), 1, Seq("aa"), 2))),
      HashJoinTestCase("UNICODE", "aa", "AA", Seq(Row(Seq("aa"), 1, Seq("aa"), 2))),
      HashJoinTestCase(
        "UNICODE_CI",
        "aa",
        "AA",
      ...
      withTable(t1, t2) {
        withSQLConf(SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> broadcastJoinThreshold.toString) {
          ...
          if (isSortMergeForced) {
            ...
            if (!CollationFactory.fetchCollation(t.collation).supportsBinaryEquality) {
              assert(
                collectFirst(queryPlan) { case b: BroadcastHashJoinExec =>
                  b.leftKeys.head
                }.head
                  .asInstanceOf[ArrayTransform]
                  .function
                  .asInstanceOf[LambdaFunction]
                  .function
                  .isInstanceOf[CollationKey])
            } else {
              assert(!collectFirst(queryPlan) { case b: BroadcastHashJoinExec =>
                b.leftKeys.head
  ...
  test(
    "hash join should be used for arrays of arrays of collated strings " +
    ...
    case class HashJoinTestCase[R](collation: String, data1: String, data2: String, result: R)
    val testCases = Seq(
      HashJoinTestCase("UTF8_BINARY", "aa", "AA", Seq(Row(Seq(Seq("aa")), 1, Seq(Seq("aa")), 2))),
      HashJoinTestCase(
        "UTF8_LCASE",
        "aa",
        "AA",
        Seq(
          Row(Seq(Seq("aa")), 1, Seq(Seq("AA")), 2),
          Row(Seq(Seq("aa")), 1, Seq(Seq("aa")), 2))),
      HashJoinTestCase("UNICODE", "aa", "AA", Seq(Row(Seq(Seq("aa")), 1, Seq(Seq("aa")), 2))),
      HashJoinTestCase(
        "UNICODE_CI",
        "aa",
        "AA",
    ...
    for {
      ...
      withTable(t1, t2) {
        withSQLConf(SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> broadcastJoinThreshold.toString) {
          sql(
            s"CREATE TABLE $t1 (x ARRAY<ARRAY<STRING COLLATE ${t.collation}>>, i int) USING " +
              s"PARQUET")
          sql(s"INSERT INTO $t1 VALUES (array(array('${t.data1}')), 1)")

          sql(
            s"CREATE TABLE $t2 (y ARRAY<ARRAY<STRING COLLATE ${t.collation}>>, j int) USING " +
              s"PARQUET")
          sql(
            s"INSERT INTO $t2 VALUES (array(array('${t.data2}')), 2)," +
  ...
  test("hash and sort merge join should respect collation for struct of strings") {
    ...
    case class HashJoinTestCase[R](collation: String, data1: String, data2: String, result: R)
    val testCases = Seq(
      HashJoinTestCase("UTF8_BINARY", "aa", "AA", Seq(Row(Row("aa"), 1, Row("aa"), 2))),
      HashJoinTestCase(
        "UTF8_LCASE",
        "aa",
        "AA",
        Seq(Row(Row("aa"), 1, Row("AA"), 2), Row(Row("aa"), 1, Row("aa"), 2))),
      HashJoinTestCase("UNICODE", "aa", "AA", Seq(Row(Row("aa"), 1, Row("aa"), 2))),
      HashJoinTestCase(
        "UNICODE_CI",
        "aa",
        "AA",
    ...
    for {
      ...
      withTable(t1, t2) {
        withSQLConf(SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> broadcastJoinThreshold.toString) {
          sql(
            s"CREATE TABLE $t1 (x STRUCT<f:STRING COLLATE ${t.collation}>, i int) USING PARQUET")
          sql(s"INSERT INTO $t1 VALUES (named_struct('f', '${t.data1}'), 1)")

          sql(
            s"CREATE TABLE $t2 (y STRUCT<f:STRING COLLATE ${t.collation}>, j int) USING PARQUET")
          sql(
            s"INSERT INTO $t2 VALUES (named_struct('f', '${t.data2}'), 2)," +
  ...
  test(
    "hash and sort merge join should respect collation " +
    ...
    for {
      ...
      withTable(t1, t2) {
        withSQLConf(SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> broadcastJoinThreshold.toString) {
          sql(
            s"CREATE TABLE $t1 (x STRUCT<f:ARRAY<STRUCT<f:STRING COLLATE ${t.collation}>>>, " +
              s"i int) USING PARQUET")
          sql(
            s"INSERT INTO $t1 VALUES (named_struct('f', array(named_struct('f', " +
              s"'${t.data1}'))), 1)")

          sql(
            s"CREATE TABLE $t2 (y STRUCT<f:ARRAY<STRUCT<f:STRING COLLATE ${t.collation}>>>, " +
  ...
  test("rewrite with collationkey should be a non-excludable rule") {
    ...
    withTable(t1, t2) {
      withSQLConf(SQLConf.OPTIMIZER_EXCLUDED_RULES.key -> collationRewriteJoinRule) {
        ...
        assert(collectFirst(queryPlan) { case _: HashJoin =>
          ()
        }.nonEmpty)
        assert(collectFirst(queryPlan) { case _: SortMergeJoinExec =>
          ()
        }.isEmpty)
      }
    }
  ...
  test("rewrite with collationkey shouldn't disrupt multiple join conditions") {
    ...
    val testCases = Seq(
      ...
      HashMultiJoinTestCase(
        "STRING COLLATE UTF8_LCASE",
        "STRING COLLATE UNICODE_CI_RTRIM",
        "'a', 'a', 1",
        "'A', 'A ', 1",
        Row("a", "a", 1, "A", "A ", 1)))
  ...
  test("inline COLLATE expressions in join conditions should not use nested loop join") {
    withTable("table1", "table2", "table3") {
      ...
      val df = sql("""SELECT t1.col1 COLLATE UTF8_LCASE_RTRIM AS result
          |FROM table1 t1
      ...
      val queryPlan = df.queryExecution.executedPlan
      assert(collectFirst(queryPlan) { case _: BroadcastNestedLoopJoinExec =>
        ()
      }.isEmpty)
    }
  }
  ...
  test("cache table with collated columns") {
    ...
    for (collation <- collations;
      ...
      def checkCacheTable(values: String): Unit = {
        ...
        assert(all.queryExecution.executedPlan.collectFirst { case _: InMemoryTableScanExec =>
          true
        }.nonEmpty)
  ...
  test("TVF collations()") {
    ...
    checkAnswer(
      sql("SELECT * FROM collations() WHERE NAME LIKE '%zh_Hant_HKG%'"),
      Seq(
        Row(
          "SYSTEM",
          "BUILTIN",
    ...
    checkAnswer(
      sql("SELECT * FROM collations() WHERE COUNTRY = 'Singapore'"),
      Seq(
        Row(
          "SYSTEM",
          "BUILTIN",
    ...
    checkAnswer(
      sql(
        "SELECT * FROM collations() WHERE LANGUAGE = 'English' " +
    ...
    checkAnswer(
      sql(
        "SELECT NAME, LANGUAGE, ACCENT_SENSITIVITY, CASE_SENSITIVITY " +
          "FROM collations() WHERE COUNTRY = 'United States'"),
      Seq(
        Row("en_USA", "English", "ACCENT_SENSITIVE", "CASE_SENSITIVE"),
    ...
    checkAnswer(
      sql("SELECT NAME FROM collations() WHERE ICU_VERSION is null"),
      Seq(
        Row("UTF8_BINARY"),
        Row("UTF8_LCASE"),
        Row("UTF8_BINARY_RTRIM"),
  ...
  test("fully qualified name") {
    Seq("UTF8_BINARY", "UTF8_LCASE", "UNICODE", "UNICODE_CI_AI").foreach { collation =>
      ...
      Seq("contains", "startswith", "endswith").foreach { binaryFunction =>
        val dfRegularName =
          sql(s"SELECT $binaryFunction('a' collate $collation, 'A' collate $collation)")
    ...
    Seq(
      ...
      ("builtin.UTF8_LCASE", "UTF8_LCASE")).foreach { case (collationName, proposal) =>
      checkError(
        ...
        sqlState = "42704",
        parameters =
          Map("collationName" -> collationName.split("\\.").last, "proposals" -> proposal))
    }

    // Case insensitive fully qualified names are supported.
    checkAnswer(sql("SELECT 'a' collate sYstEm.bUiltIn.utf8_lCAse = 'A'"), Seq(Row(true)))
  ...
  test("null aware anti join from NOT IN with collated columns") {
    ...
    withTable(tableName1, tableName2) {
      ...
      checkAnswer(
        sql(s"SELECT * FROM $tableName1 WHERE C1 NOT IN (SELECT * FROM $tableName2)"),
        expectedAnswer)
      ...
      checkAnswer(
        sql(
          s"SELECT * FROM $tableName1 WHERE C1 NOT IN (SELECT * FROM $tableName2)" +
            s" AND C1 = 'B  '"),
        Row("b"))
      checkAnswer(
        sql(
  ...
  test("null aware anti join from NOT IN with collated columns in struct type") {
    ...
    withTable(tableName1, tableName2) {
      sql(
        s"CREATE TABLE $tableName1 (C1 STRUCT<x: STRING COLLATE UTF8_LCASE_RTRIM," +
          s" y: STRING COLLATE UTF8_LCASE_RTRIM>)")
      sql(
        s"CREATE TABLE $tableName2 (C1 STRUCT<x: STRING COLLATE UTF8_LCASE_RTRIM," +
          s" y: STRING COLLATE UTF8_LCASE_RTRIM>)")
