...
class PythonStreamingDataSourceSimpleSuite extends PythonDataSourceSuiteBase {
  ...
  test("read() method throw error in SimpleDataSourceStreamReader") {
    ...
    testMicroBatchStreamError("latestOffset", "Exception: error reading available data") {
      stream =>
        stream.latestOffset(
          PythonStreamingSourceOffset("""{"partition": 0}"""),
...
class PythonStreamingDataSourceSuite extends PythonDataSourceSuiteBase {
  ...
  test("Test PythonMicroBatchStream") {
    ...
    val stream =
      new PythonMicroBatchStream(pythonDs, dataSourceName, inputSchema, options, runner)
  ...
  test("Method not implemented in stream reader") {
    ...
    testMicroBatchStreamError(
      ...
      val readLimit =
        PythonStreamingSourceReadLimit(PythonStreamingSourceRunner.READ_ALL_AVAILABLE_JSON)
  ...
  test("Error in stream reader") {
    ...
    def testMicroBatchStreamError(action: String, msg: String)(
      ...
      val stream =
        new PythonMicroBatchStream(pythonDs, errorDataSourceName, inputSchema, options, runner)
      val err = intercept[SparkException] {
        func(stream)
      }
      checkErrorMatchPVals(
  ...
  test("empty batch for stream when latestOffset produces the same offset with start") {
    ...
    val q = unionDF.writeStream
      .foreachBatch((df: DataFrame, batchId: Long) => {
        ...
        assert(rateRows == 2, s"Batch $batchId: Expected 2 rows from rate source, got $rateRows")
