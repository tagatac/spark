...
class QueryParsingErrorsSuite extends QueryTest with SharedSparkSession with SQLHelper {
  ...
  test("INVALID_SQL_SYNTAX.INVALID_TABLE_VALUED_FUNC_NAME: Invalid table value function name") {
    checkError(
      ...
      context = ExpectedContext(fragment = "db.func()", start = 14, stop = 22))
    ...
    checkError(
      ...
      context = ExpectedContext(fragment = "ns.db.func()", start = 14, stop = 25))
  }
  ...
  test("INVALID_SQL_SYNTAX.MULTI_PART_NAME: Create temporary function with multi-part name") {
    ...
    checkError(
      ...
      sqlState = "42000",
      parameters = Map("statement" -> "CREATE TEMPORARY FUNCTION", "name" -> "`ns`.`db`.`func`"),
      context = ExpectedContext(fragment = sqlText, start = 0, stop = 132))
  }

  test(
    "INVALID_SQL_SYNTAX.CREATE_TEMP_FUNC_WITH_DATABASE: " +
    ...
    checkError(
      ...
      context = ExpectedContext(fragment = sqlText, start = 0, stop = 129))
  }

  test(
    "INVALID_SQL_SYNTAX.MULTI_PART_NAME: Drop temporary function requires a single part name") {
    val sqlText = "DROP TEMPORARY FUNCTION db.func"
    checkError(
      exception = parseException(sqlText),
      condition = "INVALID_SQL_SYNTAX.MULTI_PART_NAME",
      sqlState = "42000",
      parameters = Map("statement" -> "DROP TEMPORARY FUNCTION", "name" -> "`db`.`func`"),
      context = ExpectedContext(fragment = sqlText, start = 0, stop = 30))
  }
