 ...
 *
 * This allows subclasses to plugin a custom `SparkSession`. It comes with test data prepared in
 * advance as well as all implicit conversions used extensively by dataframes. To use implicit
 * methods, import `testImplicits._` instead of through the `SparkSession`.
 *
 * Subclasses should *not* create `SparkSession`s in the test suite constructor, which is prone to
...
private[sql] trait SQLTestUtils extends SparkFunSuite with SQLTestUtilsBase with PlanTest {
  ...
  protected def withTempPaths(numPaths: Int)(f: Seq[File] => Unit): Unit = {
    ...
    try f(files.toImmutableArraySeq)
    finally {
