...
import org.apache.spark.sql.pipelines.graph.DataflowGraphTransformer.{TransformNodeFailedException, TransformNodeRetryableException}
...
class CoreDataflowNodeProcessor(rawGraph: DataflowGraph) {
   ...
   * Takes care of resolving the flows and virtualizing tables (i.e. removing tables to ensure
   * resolution is internally consistent) if needed for the nodes.
   * @param node
   *   The node to process
   * @param upstreamNodes
   *   Upstream nodes for the node
   * @return
