...
object PipelinesErrors extends Logging {
   ...
   *
   * @param originalErr
   *   The error on which getCause is repeatedly called
   * @return
   *   An ArrayBuffer containing the original error and all the causes in its exception chain. For
   *   a given exception in the ArrayBuffer, the next element is its cause.
   */
   ...
   * @param throwable
   *   A Throwable to inspect
   * @param check
   *   Function to run on each cause
   * @return
   *   Whether or not `throwable` or any of its nested causes satisfy the check
   */
   ...
   * @param env
   *   the update context
   * @param graphExecution
   *   the graph execution
   * @param flow
   *   the resolved logical flow
   * @param shouldRethrow
   *   whether to throw an UpdateTerminationException wrapping `ex`. This is set to true for
   *   ContinuousFlowExecution so we can eagerly stop the execution.
   * @param prevFailureCount
   *   the number of failures that have occurred so far
   * @param maxRetries
   *   the max retries that were available (whether or not they're exhausted now)
   */
 ...
 *
 * @param directFailures
 *   Mapping between the name of flows that failed to resolve (due to an error in that flow) and
 *   the error that occurred when attempting to resolve them
 * @param downstreamFailures
 *   Mapping between the name of flows that failed to resolve (because they failed to read from
 *   other unresolved flows) and the error that occurred when attempting to resolve them
 */
case class UnresolvedPipelineException(
    ...
    extends AnalysisException(s"""
       ...
       |Flows with errors: ${directFailures.keys
...
/**
 * Raised when there's a circular dependency in the current pipeline. That is, a downstream table
 * is referenced while creating a upstream table.
 */
