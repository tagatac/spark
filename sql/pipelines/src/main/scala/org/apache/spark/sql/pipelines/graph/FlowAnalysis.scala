...
object FlowAnalysis {
  ...
  /**
   * Creates a [[FlowFunction]] that attempts to analyze the provided LogicalPlan using the
   * existing resolved inputs.
   *   - If all upstream inputs have been resolved, then analysis succeeds and the function
   *     returns a [[FlowFunctionResult]] containing the dataframe.
   *   - If any upstream inputs are unresolved, then the function throws an exception.
   *
   * @param plan
   *   The user-supplied LogicalPlan defining a flow.
   * @return
   ...
   *
   * @param plan
   *   The [[LogicalPlan]] defining a flow.
   * @return
   *   An analyzed [[DataFrame]].
   */
  private def analyze(context: FlowAnalysisContext, plan: LogicalPlan): DataFrame = {
    ...
    val resolvedPlan = planWithInlinedCTEs transformWithSubqueries {
      ...
      case u: UnresolvedRelation if u.isStreaming =>
        val resolved = readStreamInput(
          ...
          spark.readStream,
          streamingReadOptions = StreamingReadOptions()).queryExecution.analyzed
      ...
      case u: UnresolvedRelation =>
        val resolved = readBatchInput(
          context,
          name = IdentifierHelper.toQuotedString(u.multipartIdentifier),
          batchReadOptions = BatchReadOptions()).queryExecution.analyzed
        // Spark Connect requires the PLAN_ID_TAG to be propagated to the resolved plan
        // to allow correct analysis of the parent plan that contains this subquery
        resolved.mergeTagsFrom(u)
        resolved
   ...
   *   2. The dataset can be a dataset defined in the same DataflowGraph or a table in the
   *      external catalog. All the public APIs that read from a dataset should call this function
   *      to read the dataset.
   *
   * @param name
   *   the name of the Dataset to be read.
   * @param batchReadOptions
   *   Options for this batch read
   * @return
   *   batch DataFrame that represents data from the specified Dataset.
   */
  final private def readBatchInput(
    ...
    GraphIdentifierManager.parseAndQualifyInputIdentifier(context, name) match {
      ...
      case inputIdentifier: ExternalDatasetIdentifier =>
        readExternalBatchInput(context, inputIdentifier = inputIdentifier, name = name)
    }
  }
   ...
   *   the name of the Dataset to be read.
   * @param streamReader
   *   The [[DataStreamReader]] that may hold read options specified by the user.
   * @param streamingReadOptions
   *   Options for this streaming read.
   * @return
   *   streaming DataFrame that represents data from the specified Dataset.
   */
  final private def readStreamInput(
    ...
    GraphIdentifierManager.parseAndQualifyInputIdentifier(context, name) match {
      case inputIdentifier: InternalDatasetIdentifier =>
        readGraphInput(context, inputIdentifier, streamingReadOptions)
   ...
   *   The identifier of the Dataset to be read.
   * @param readOptions
   *   Options for this read (may be either streaming or batch options)
   * @return
   *   streaming or batch DataFrame that represents data from the specified Dataset.
   */
  ...
  /**
   * Internal helper to reference batch dataset (i.e., non-streaming dataset) defined in an
   * external catalog or as a path.
   *
   * @param inputIdentifier
   *   The identifier of the dataset to be read.
   * @return
   ...
   *   The identifier of the dataset to be read.
   * @param streamReader
   *   The [[DataStreamReader]] that may hold additional read options specified by the user.
   * @return
   *   streaming or batch DataFrame that represents data from the specified Dataset.
   */
