...
class SqlQueryOriginSuite extends PipelineTest with SharedSparkSession {
  test("basic test") {
    val sqlQueryOrigins = SqlGraphRegistrationContext
      .splitSqlFileIntoQueries(
        spark,
        sqlFilePath = "file.sql",
        sqlFileText = """-- comment 1
    ...
    assert(
      sqlQueryOrigins == Seq(
        QueryOrigin(
          sqlText = Option("""CREATE MATERIALIZED VIEW a.b.c AS SELECT 1""".stripMargin),
          filePath = Option("file.sql"),
          language = Option(Language.Sql()),
          startPosition = Option(13),
          line = Option(2)),
        QueryOrigin(
          sqlText = Option("""USE DATABASE d """.stripMargin),
          filePath = Option("file.sql"),
          language = Option(Language.Sql()),
  ...
  test("\\n in sql file is not considered a new line") {
    val sqlQueryOrigins = SqlGraphRegistrationContext
      .splitSqlFileIntoQueries(
        spark,
        sqlFilePath = "file.sql",
        sqlFileText = """CREATE STREAMING TABLE `a.\n` AS SELECT "\n";
  ...
  test("White space is accounted for in startPosition") {
    val sqlQueryOrigins = SqlGraphRegistrationContext
      ...
      .map(_.queryOrigin)
    assert(
      sqlQueryOrigins == Seq(QueryOrigin(
  ...
  test("Preceeding comment is ommitted from sql text") {
    ...
    assert(
      sqlQueryOrigins == Seq(QueryOrigin(
        sqlText = Option(s"""CREATE MATERIALIZED VIEW mv -- another comment
