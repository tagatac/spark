...
class CompressionCodecSuite extends TestHiveSingleton with ParquetTest {
  ...
  private def writeDataToTable(tableName: String, partitionValue: Option[String]): Unit = {
  ...
  private def writeDataToTableUsingCTAS(
    ...
    sql(s"""
        ...
        |USING $format
        |OPTIONS('path'='${rootDir.toURI.toString.stripSuffix(
            "/")}/$tableName' $compressionOption)
        |$partitionCreate
  ...
  private def checkCompressionCodecForTable(
      ...
      isPartitioned: Boolean,
      compressionCodec: Option[String],
      usingCTAS: Boolean)(assertion: (String, Long) => Unit): Unit = {
    val tableName =
        ...
        withTempDir { tmpDir =>
          ...
          withTable(tableName) {
            compressCodecs.foreach { compressionCodec =>
              ...
              withSQLConf(
                getConvertMetastoreConfName(format) -> convertMetastore.toString,
