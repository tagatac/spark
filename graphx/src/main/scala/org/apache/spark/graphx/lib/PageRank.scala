 ...
 *
 * The first implementation uses the standalone `Graph` interface and runs PageRank for a fixed
 ...
 *
 * @note
 *   This is not the "normalized" PageRank and as a consequence pages that have no inlinks will
 *   have a PageRank of alpha.
 */
object PageRank extends Logging {
  ...
  /**
   * Run an update pass of PageRank algorithm. Update the values of every node in the pageRank
   *
   * @param rankGraph
   *   the current PageRank
   * @param personalized
   *   True if personalized pageRank
   * @param resetProb
   *   the random reset probability (alpha)
   * @param src
   *   the source vertex for a Personalized Page Rank
   *
   * @return
   *   the graph containing with each vertex containing the PageRank and each edge containing the
   *   normalized weight after a single update step.
   */
  private def runUpdate(
      rankGraph: Graph[Double, Double],
      personalized: Boolean,
      resetProb: Double,
      src: VertexId): Graph[Double, Double] = {
  ...
  def runWithOptions[VD: ClassTag, ED: ClassTag](
      ...
      normalized: Boolean): Graph[Double, Double] = {
    require(
      numIter > 0,
  ...
  def runWithOptionsWithPreviousPageRank[VD: ClassTag, ED: ClassTag](
    ...
    require(
      ...
      s"Graph and previous pageRankGraph" +
   ...
   * parallel. Returns a graph with vertex attributes containing the pagerank relative to all
   * starting nodes (as a sparse vector) and edge attributes the normalized edge weight
   *
   * @tparam VD
   *   The original vertex attribute (not used)
   * @tparam ED
   ...
   * @return
   *   the graph with vertex attributes containing the pagerank relative to all starting nodes (as
   *   a sparse vector indexed by the position of nodes in the sources list) and edge attributes
  ...
  def runParallelPersonalizedPageRank[VD: ClassTag, ED: ClassTag](
    ...
    require(
      sources.nonEmpty,
      s"The list of sources must be non-empty," +
  ...
  def runUntilConvergenceWithOptions[VD: ClassTag, ED: ClassTag](
    ...
    val vp = if (personalized) { (id: VertexId, attr: (Double, Double), msgSum: Double) =>
