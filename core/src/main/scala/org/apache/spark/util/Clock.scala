...
private[spark] trait Clock {
   ...
   *
   * This method abstracts the call to the JRE's `System.nanoTime()` call. As with that method,
   * the value here is not guaranteed to be monotonically increasing, but rather a higher
   * resolution time source for use in the calculation of time intervals. The characteristics of
   * the values returned may very from JVM to JVM (or even the same JVM running on different OSes
   * or CPUs), but in general it should be preferred over [[getTimeMillis()]] when calculating
   * time differences.
   *
  ...
  /**
   * Wait until the wall clock reaches at least the given time. Note this may not actually wait
   * for the actual difference between the current and target times, since the wall clock may
   * drift.
   */
...
private[spark] class SystemClock extends Clock with Serializable {
  ...
  /**
   * @return
   *   the same time (milliseconds since the epoch) as is reported by `System.currentTimeMillis()`
   */
  override def getTimeMillis(): Long = System.currentTimeMillis()

  /**
   * @return
   *   value reported by `System.nanoTime()`.
   */
  override def nanoTime(): Long = System.nanoTime()

  /**
   * @param targetTime
   *   block until the current time is at least this value
   * @return
   *   current system time when wait has completed
   */
