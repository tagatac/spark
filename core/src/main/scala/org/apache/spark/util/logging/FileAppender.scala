...
private[spark] class FileAppender(
  ...
  /**
   * Wait for the appender to stop appending, either because input stream is closed or because of
   * any error in appending
   */
...
/**
 * Companion object to [[org.apache.spark.util.logging.FileAppender]] which has helper functions
 * to choose the correct type of FileAppender based on SparkConf configuration.
 */
private[spark] object FileAppender extends Logging {
  ...
  def apply(
      ...
      file: File,
      conf: SparkConf,
      closeStreams: Boolean = false): FileAppender = {
    ...
    def createTimeBasedAppender(): FileAppender = {
      val validatedParams: Option[(Long, String)] = rollingInterval match {
        case "daily" =>
          logInfo(
            log"Rolling executor logs enabled for ${MDC(FILE_NAME, file)} with daily rolling")
        ...
        case _ =>
          logWarning(
            log"Illegal interval for rolling executor logs [" +
