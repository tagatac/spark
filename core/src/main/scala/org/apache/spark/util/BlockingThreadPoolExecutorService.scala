...
// scalastyle:off
/**
 * This thread pool executor throttles the submission of new tasks by using a semaphore. Task
 * submissions require permits, task completions release permits. <p> NOTE: [[invoke*]] methods
 * are not supported, you should either use the [[submit]] methods or the [[execute]] method. <p>
...
private[spark] class BlockingThreadPoolExecutorService(
  ...
  private class RunnableWithPermitRelease(delegate: Runnable) extends Runnable {
    override def run(): Unit = try delegate.run()
  ...
  private class CallableWithPermitRelease[T](delegate: Callable[T]) extends Callable[T] {
    override def call(): T = try delegate.call()
    finally permits.release()
  }
}
