...
private[spark] class PythonAccumulatorV2(
  ...
  // TPC socket
  def this(serverHost: String, serverPort: Int, secretToken: String) =
    this(Some(serverHost), Some(serverPort), Some(secretToken), None)
  ...
  private def openSocket(): SocketChannel = synchronized {
    if (socket == null || !socket.isOpen) {
      if (isUnixDomainSock) {
        ...
        logInfo(
          log"Connected to AccumulatorServer at host: ${MDC(HOST, serverHost.get)}" +
