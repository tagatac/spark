 ...
 *
 * When delegation token renewal is enabled, this manager will make sure long-running apps can run
 ...
 *
 * Renewal can be enabled in two different ways: by providing a principal and keytab to Spark, or
 * by enabling renewal based on the local credential cache. The latter has the drawback that Spark
...
private[spark] class HadoopDelegationTokenManager(
    ...
    protected val schedulerRef: RpcEndpointRef)
    extends Logging {
  ...
  logDebug(
    "Using the following builtin delegation token providers: " +
  ...
  /**
   * Start the token renewer. Requires a principal and keytab. Upon start, the renewer will obtain
   * delegation tokens for all configured services and send them to the driver, and set up tasks
   * to periodically get fresh tokens as needed.
   *
   ...
   *
   * @return
   *   New set of delegation tokens created for the configured principal.
   */
   ...
   *
   * @param creds
   *   Credentials object where to store the delegation tokens.
   */
   ...
   *
   * @return
   *   2-tuple (credentials with new tokens, time by which the tokens must be renewed)
   */
  private def obtainDelegationTokens(): (Credentials, Long) = {
    val creds = new Credentials()
    val nextRenewal = delegationTokenProviders.values
      .flatMap { provider =>
        if (provider.delegationTokensRequired(sparkConf, hadoopConf)) {
          provider.obtainDelegationTokens(hadoopConf, sparkConf, creds)
        } else {
          logDebug(
            s"Service ${provider.serviceName} does not require a token." +
  ...
  private def scheduleRenewal(delay: Long): Unit = {
    ...
    logInfo(
      log"Scheduling renewal in ${MDC(LogKeys.TIME_UNITS, UIUtils.formatDuration(_delay))}.")
  ...
  /**
   * Periodic task to login to the KDC and create new delegation tokens. Re-schedules itself to
   * fetch the next set of tokens when needed.
   */
  private def updateTokensTask(): Array[Byte] = {
    try {
      ...
      case e: Exception =>
        ...
        logWarning(
          log"Failed to update tokens, will try again in " +
            log"${MDC(LogKeys.TIME_UNITS, UIUtils.formatDuration(delay))}!" +
            log" If this happens too often tasks will fail.",
          e)
  ...
  /**
   * Obtain new delegation tokens from the available providers. Schedules a new task to fetch new
   * tokens before the new set expires.
   *
   * @return
   *   Credentials containing the new tokens.
   */
  private def obtainTokensAndScheduleRenewal(ugi: UserGroupInformation): Credentials = {
    ugi.doAs(new PrivilegedExceptionAction[Credentials]() {
      override def run(): Credentials = {
        ...
        logInfo(
          log"Calculated delay on renewal is ${MDC(LogKeys.DELAY, delay)}," +
  ...
  private def doLogin(): UserGroupInformation = {
    if (principal != null) {
      logInfo(
        log"Attempting to login to KDC using principal: ${MDC(LogKeys.PRINCIPAL, principal)}")
      ...
      val user = Option(sparkConf.getenv("KRB5PRINCIPAL"))
...
private[spark] object HadoopDelegationTokenManager extends Logging {
  ...
  private val deprecatedProviderEnabledConfigs =
    List("spark.yarn.security.tokens.%s.enabled", "spark.yarn.security.credentials.%s.enabled")
  ...
  def isServiceEnabled(sparkConf: SparkConf, serviceName: String): Boolean = {
    ...
    deprecatedProviderEnabledConfigs.foreach { pattern =>
      ...
      if (sparkConf.contains(deprecatedKey)) {
        logWarning(
          log"${MDC(LogKeys.DEPRECATED_KEY, deprecatedKey)} is deprecated. " +
