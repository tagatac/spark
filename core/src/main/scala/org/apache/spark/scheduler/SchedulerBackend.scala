 ...
 * TaskSchedulerImpl. We assume a model where the application gets resource offers as machines
 * become available and can launch tasks on them.
 */
private[spark] trait SchedulerBackend {
   ...
   * Applications run in client mode will not have attempt IDs.
   *
   * @return
  ...
  /**
   * Get the attributes on driver. These attributes are used to replace log URLs when custom log
   * url pattern is specified.
   * @return
   ...
   *
   * Currently push based shuffle is disabled for both stage retry and stage reuse cases (for eg:
   * in the case where few partitions are lost due to failure). Hence this method should be
   * invoked only once for a ShuffleDependency.
   * @return
