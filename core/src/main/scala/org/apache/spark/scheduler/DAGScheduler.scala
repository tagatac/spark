...
private[spark] class DAGScheduler(
   ...
   *
   * All accesses to this map should be guarded by synchronizing on it (see SPARK-4454). If you
   * need to access any RDD while synchronizing on the cache locations, first synchronize on the
   * RDD, and then synchronize on this map to avoid deadlocks. The RDD could try to access the
   * cache locations after synchronizing on the RDD.
   */
  ...
  private def getMissingParentStages(stage: Stage): List[Stage] = {
    ...
    def visit(rdd: RDD[_]): Unit = {
      if (!visited(rdd)) {
        visited += rdd
        val rddHasUncachedPartitions =
          try {
            getCacheLocs(rdd).contains(Nil)
          } catch {
            case e: RpcTimeoutException =>
              logWarning(
                log"Failed to get cache locations for RDD ${MDC(RDD_ID, rdd.id)} due " +
                  log"to rpc timeout, assuming not fully cached.",
                e)
              true
          }
   ...
   * @param cancelledJobs
   *   a promise to be completed with operation IDs being cancelled.
   */
  ...
  private[scheduler] def handleJobSubmitted(
    ...
    logInfo(
      log"Final stage: ${MDC(STAGE, finalStage)} " +
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        if (stage.getNextAttemptId >= maxStageAttempts) {
          ...
          logInfo(
            log"Missing parents found for ${MDC(STAGE, stage)}: ${MDC(MISSING_PARENT_STAGES, missing)}")
          if (missing.isEmpty) {
            logInfo(
              log"Submitting ${MDC(STAGE, stage)} (${MDC(RDD_ID, stage.rdd)}), " +
  ...
  private def prepareShuffleServicesForShuffleMapStage(stage: ShuffleMapStage): Unit = {
    ...
    if (stage.shuffleDep.shuffleMergeEnabled) {
      logInfo(
        log"Shuffle merge enabled before starting the stage for ${MDC(STAGE, stage)}" +
      ...
      logInfo(
        log"Shuffle merge disabled for ${MDC(STAGE, stage)} with " +
      ...
      case s: ShuffleMapStage =>
        ...
        if (s.shuffleDep.shuffleMergeAllowed) {
          if (!s.shuffleDep.isShuffleMergeFinalizedMarked) {
            ...
            logInfo(
              log"Push-based shuffle disabled for ${MDC(STAGE, stage)} " +
   ...
   * when re-try. To make sure data correctness, we need to clean up shuffles to make sure
   * succeeding stages will be resubmitted and re-try all the tasks, as the input data may be
   * changed after the map tasks are re-tried. For stages where rollback and retry all tasks are
   * not possible, we will need to abort the stages.
   *
   * @return
   *   true if the corresponding active jobs are not aborted and will continue to run after
  ...
  private[scheduler] def rollbackSucceedingStages(
    ...
    logInfo(
      log"Found succeeding stages ${MDC(STAGES, stagesToRollback)} of " +
    ...
    if (!hasActiveJobs) {
      logInfo(
        log"All jobs depending on the indeterminate stage " +
      ...
      stagesToRollback.toSeq.sortBy(!runningStages.contains(_)).foreach {
        case sms: ShuffleMapStage =>
          ...
          if (sms.getNextAttemptId > 0 && !alreadyRollingBack) {
            rollbackShuffleMapStage(
              sms,
              "rolling back due to indeterminate " +
      ...
      logInfo(
        log"The shuffle map stage ${MDC(STAGE, mapStage)} with indeterminate output " +
  ...
  private def getCompletedJobsFromSameQuery(mapStage: ShuffleMapStage): Array[ActiveJob] = {
    ...
    val executionIds = mapStage.jobIds
  ...
  private[scheduler] def rollbackSucceedingStagesForQuery(mapStage: ShuffleMapStage): Unit = {
    ...
    logInfo(
      log"Found succeeding stages ${MDC(STAGES, succeedingStagesInCompletedJobs)} of " +
    ...
    if (hasActiveJobs) {
      ...
      succeedingStagesInCompletedJobs
        .collect { case s: ShuffleMapStage => s }
        .foreach(stage =>
          rollbackShuffleMapStage(
            stage,
            "rolling back due to indeterminate " +
      ...
      logInfo(
        log"All jobs depending on the checksum mismatch stage " +
   ...
   *   1. If the stage is running, cancel the stage and kill all running tasks. Clean up the
   *      shuffle output resubmit it if it's not exceeded max retries.
   ...
   *
   * @param sms
   *   the shuffle map stage to roll back
   * @param reason
  ...
  private def rollbackShuffleMapStage(sms: ShuffleMapStage, reason: String): Unit = {
    ...
    val clearShuffle = if (runningStages.contains(sms)) {
      logInfo(
        log"Stage ${MDC(STAGE, sms)} is running, marking it as failed and " +
   ...
   *
   * @param stage
   *   the stage to cancel and resubmit
   * @param reason
   *   the reason for the operation
   * @return
   *   true if the stage is successfully cancelled and resubmitted, otherwise false
   */
  private def cancelStageAndTryResubmit(stage: ShuffleMapStage, reason: String): Boolean = {
    ...
    try {
      ...
      case e: UnsupportedOperationException =>
        ...
        abortStage(
          stage,
          "Rollback failed due to: Not able to kill running tasks for stage " +
            s"$stage (${stage.name})",
          Some(e))
        return false
    }
    ...
    if (shouldAbortStage) {
      ...
      if (noResubmitEnqueued) {
        logInfo(
          log"Resubmitting ${MDC(FAILED_STAGE, stage)} " +
                      ...
                      case e: UnsupportedOperationException =>
                        logWarning(
                          log"Could not cancel tasks " +
                            log"for stage ${MDC(STAGE, stageId)}",
                          e)
                    }
                    listenerBus.post(
          ...
          case smt: ShuffleMapTask =>
            ...
            if (!ignoreOldTaskAttempts) {
              ...
              if (executorFailureEpoch.contains(execId) &&
                ...
                logInfo(
                  log"Ignoring possibly bogus ${MDC(STAGE, smt)} completion from " +
        ...
        if (failedStage.latestInfo.attemptNumber() != task.stageAttemptId) {
          ...
          if (shouldAbortStage) {
            ...
            if (noResubmitEnqueued) {
              ...
              logInfo(log"Resubmitting ${MDC(STAGE, mapStage)} " +
  ...
  private def collectSucceedingStagesByShuffleId(
      mapStage: ShuffleMapStage,
      jobs: Array[ActiveJob]): HashSet[Stage] = {
   ...
   *
   * @param stagesToRollback
   *   stages to roll back
   * @return
   *   Shuffle map stages which need and can be rolled back
   */
  ...
  private[scheduler] def handleShuffleStatusNotFoundException(
    ...
    if (stage.isDefined) {
      ...
      logWarning(
        s"Aborting stage because of $reason. It is possible that the stage is " +
          "being cancelled.")
    } else {
      logWarning(
        s"Tried aborting stage because of $reason, but the stage was not found. " +
  ...
  private def failJobAndIndependentStages(job: ActiveJob, error: Exception): Unit = {
