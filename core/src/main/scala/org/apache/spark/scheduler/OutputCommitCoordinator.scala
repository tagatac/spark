...
private[spark] class OutputCommitCoordinator(conf: SparkConf, isDriver: Boolean) extends Logging {
   ...
   *
   * Entries are added to the top-level map when stages start and are removed they finish (either
   ...
   * @param attemptNumber
   *   how many times this task has been attempted (see [[TaskContext.attemptNumber()]])
   * @return
   *   true if this task is authorized to commit, false otherwise
   */
   ...
   * @param stage
   *   the stage id.
   * @param maxPartitionId
   *   the maximum partition id that could appear in this stage's tasks (i.e. the maximum possible
   *   value of `context.partitionId`).
   */
      ...
      case AskPermissionToCommitOutput(stage, stageAttempt, partition, attemptNumber) =>
        context.reply(
          outputCommitCoordinator
            .handleAskPermissionToCommit(stage, stageAttempt, partition, attemptNumber))
    }
  }
}
