 ...
 * @param partitionId
 *   index of the number in the RDD
 * @param numPartitions
...
private[spark] abstract class Task[T](
   ...
   *
   * @param taskAttemptId
   *   an identifier for this task attempt that is unique within a SparkContext.
   * @param attemptNumber
   *   how many times this task has been attempted (0 for the first attempt)
   * @param resources
   *   other host resources (like gpus) that this task attempt can access
   * @return
   *   the result of the task along with updates of Accumulators.
   */
  final def run(
      taskAttemptId: Long,
      attemptNumber: Int,
  ...
  /**
   * Collect the latest values of accumulators used in this task. If the task failed, filter out
   * the accumulators whose values should not be included on failures.
   */
  def collectAccumulatorUpdates(taskFailed: Boolean = false): Seq[AccumulatorV2[_, _]] = {
    if (context != null) {
      ...
      context.taskMetrics().nonZeroInternalAccums() ++
        ...
        context
          .taskMetrics()
          .withExternalAccums(_.filter(a => !taskFailed || a.countFailedValues))
    } else {
      Seq.empty
    }
  }
   ...
   * be called multiple times. If interruptThread is true, we will also call Thread.interrupt() on
   * the Task's executor thread.
   */
    ...
    if (context != null) {
      TaskContext.synchronized {
        if (context.interruptible()) {
          ...
          logInfo(
            log"Task ${MDC(LogKeys.TASK_ID, context.taskAttemptId())} " +
