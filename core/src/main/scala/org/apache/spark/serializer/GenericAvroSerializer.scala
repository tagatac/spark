...
/**
 * Custom serializer used for generic Avro containers. If the user registers the schemas ahead of
 * time, then the schema's fingerprint will be sent with each message instead of the actual
 * schema, as to reduce network IO. Actions like parsing or compressing schemas are
 * computationally expensive so the serializer caches all previously seen values as to reduce the
 * amount of work needed to do.
 * @param schemas
 *   a map where the keys are unique IDs for Avro schemas and the values are the string
 *   representation of the Avro schema, used to decrease the amount of data that needs to be
 *   serialized.
 * @tparam D
 *   the subtype of [[GenericContainer]] handled by this serializer
 */
private[serializer] class GenericAvroSerializer[D <: GenericContainer](schemas: Map[Long, String])
  ...
  /**
   * Used to compress Schemas when they are being sent over the wire. The compression results are
   * memoized to reduce the compression time since the same schema is compressed many times over
   */
   ...
   */
  def decompress(schemaBytes: ByteBuffer): Schema = decompressCache.getOrElseUpdate(
    schemaBytes, {
  ...
  /**
   * Deserializes generic containers into their in-memory form. There is internal state to keep a
   * cache of already seen schemas and datum readers.
   */
