...
/**
 * Serves requests to open blocks by simply registering one chunk per block requested. Handles
 * opening and uploading arbitrary BlockManager blocks.
 *
 * Opened blocks are registered with the "one-for-one" strategy, meaning each Transport-layer
 * Chunk is equivalent to one Spark-level shuffle block.
 */
class NettyBlockRpcServer(appId: String, serializer: Serializer, blockManager: BlockDataManager)
    ...
    val message =
      try {
        ...
        case e: IllegalArgumentException if e.getMessage.startsWith("Unknown message type") =>
          logWarning(
            log"This could be a corrupted RPC message (capacity: " +
        ...
        case _: IndexOutOfBoundsException | _: NegativeArraySizeException =>
          ...
          logWarning(
            log"Ignored a corrupted RPC message (capacity: " +
              log"${MDC(RPC_MESSAGE_CAPACITY, rpcMessage.capacity())}) " +
              log"from ${MDC(SOCKET_ADDRESS, client.getSocketAddress)}. " +
              log"Please use `spark.authenticate.*` configurations " +
              log"in case of security incidents.")
    ...
    message match {
      case openBlocks: OpenBlocks =>
        ...
        val blocks = (0 until blocksNum).map { i =>
          ...
          assert(
            !blockId.isInstanceOf[ShuffleBlockBatchId],
            "Continuous shuffle block fetching only works for new fetch protocol.")
          blockManager.getLocalBlockData(blockId)
        }
        val streamId =
          streamManager.registerStream(appId, blocks.iterator.asJava, client.getChannel)
          ...
          if (!fetchShuffleBlocks.batchFetchEnabled) {
            ...
            if (startAndEndId.length != 2) {
              throw SparkException.internalError(
                "Invalid shuffle fetch request when batch mode " +
      ...
      case uploadBlock: UploadBlock =>
        ...
        logDebug(
          s"Receiving replicated block $blockId with level ${level} " +
      ...
      case getLocalDirs: GetLocalDirsForExecutors =>
        ...
        if (isIncorrectAppId || execNum != 1) {
          ...
          responseContext.onFailure(SparkException.internalError(errorMsg, category = "NETWORK"))
        } else {
  ...
  override def receiveStream(
    ...
    logDebug(
      s"Receiving replicated block $blockId with level ${level} as stream " +
        s"from ${client.getSocketAddress}")
