 ...
 *   mapIndex for this block, which indicate the index in the map stage. Note that zero-sized
 *   blocks are already excluded, which happened in
 ...
 *   whether the shuffle checksum is enabled. When enabled, Spark will try to diagnose the cause
 *   of the block corruption.
...
private[spark] final class ShuffleBlockFetcherIterator(
  ...
  /**
   * Count the retry times for the blocks due to Netty OOM. The block will stop retry if retry
   * times has exceeded the [[maxAttemptsOnNettyOOM]].
   */
  ...
  private[this] val pushBasedFetchHelper =
    new PushBasedFetchHelper(this, shuffleClient, blockManager, mapOutputTracker, shuffleMetrics)
            ...
            case _: OutOfDirectMemoryError
              ...
              if (!isZombie) {
                ...
                if (isNettyOOMOnShuffle.compareAndSet(false, true)) {
                  ...
                  logInfo(
                    log"Block ${MDC(BLOCK_ID, blockId)} has failed " +
  ...
  private[this] def partitionBlocksByFetchMode(
    ...
    logInfo(log"Getting ${MDC(NUM_BLOCKS, blocksToFetchCurrentIteration)} " +
  ...
  private[this] def initialize(): Unit = {
    ...
    logInfo(
      log"Started ${MDC(COUNT, numFetches)} remote fetches in " +
        log"${MDC(DURATION, Utils.getUsedTimeNs(startTimeNs))}" +
        (if (numDeferredRequest > 0)
           log", deferred ${MDC(NUM_REQUESTS, numDeferredRequest)} requests"
         else log""))
  ...
  private[storage] def diagnoseCorruption(
    ...
    blockId match {
      case shuffleBlock: ShuffleBlockId =>
        ...
        logInfo(
          log"Finished corruption diagnosis in ${MDC(DURATION, duration)} ms. " +
            log"${MDC(STATUS, diagnosisResponse)}")
        diagnosisResponse
      case shuffleBlockChunk: ShuffleBlockChunkId =>
        // TODO SPARK-36284 Add shuffle checksum support for push-based shuffle
        logWarning(
          log"BlockChunk ${MDC(SHUFFLE_BLOCK_INFO, shuffleBlockChunk)} " +
  ...
  private def fetchUpToMaxBytes(): Unit = {
    ...
    while (isRemoteBlockFetchable(fetchRequests)) {
      ...
      if (isRemoteAddressMaxedOut(remoteAddress, request)) {
        ...
        val defReqQueue =
          deferredFetchRequests.getOrElse(remoteAddress, new Queue[FetchRequest]())
        defReqQueue.enqueue(request)
  ...
  private[storage] def fallbackFetch(
    ...
    logInfo(
      log"Created ${MDC(NUM_REQUESTS, originalRemoteReqs.size)} fallback remote requests " +
        log"for push-merged")
  ...
  /**
   * Removes all the pending shuffle chunks that are on the same host and have the same reduceId
   * as the current chunk that had a fetch failure. This is executed by the task thread when the
...
private class BufferReleasingInputStream(
   ...
   * IOException as FetchFailedException when detectCorruption is true. This method is only used
   * by the `available`, `read` and `skip` methods inside `BufferReleasingInputStream` currently.
   */
    ...
    val result = if (doBatchFetch) {
      ...
      def mergeFetchBlockInfo(toBeMerged: ArrayBuffer[FetchBlockInfo]): FetchBlockInfo = {
        ...
        def shouldMergeIntoPreviousBatchBlockId =
          mergedBlockInfo.last.blockId
            .asInstanceOf[ShuffleBlockBatchId]
            .mapId == startBlockId.mapId
