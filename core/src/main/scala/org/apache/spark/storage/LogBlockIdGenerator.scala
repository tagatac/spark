...
trait LogBlockIdGenerator {
  ...
  /**
   * Generates a new LogBlockId based on the last log time and executor ID. Make sure that the
   * generated LogBlockId has the same log block type as this generator.
   *
   * @param lastLogTime
   *   The timestamp of the last log entry.
   * @param executorId
   *   The ID of the executor generating the log block.
   */
...
class PythonWorkerLogBlockIdGenerator(sessionId: String, workerId: String)
