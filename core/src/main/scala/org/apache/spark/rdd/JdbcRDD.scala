...
/**
 * An RDD that executes a SQL query on a JDBC connection and reads results. For usage example, see
 * test case JdbcRDDSuite.
 *
 * @param getConnection
 *   a function that returns an open Connection. The RDD takes care of closing the connection.
 * @param sql
 *   the text of the query. The query must contain two ? placeholders for parameters used to
 *   partition the results. For example,
 *   {{{
 ...
 * @param numPartitions
 *   the number of partitions. Given a lowerBound of 1, an upperBound of 20, and a numPartitions
 *   of 2, the query would be executed twice, once with (1, 10) and once with (11, 20)
 * @param mapRow
 *   a function from a ResultSet to a single row of the desired result type(s). This should only
 *   call getInt, getString, etc; the RDD takes care of calling next. The default maps a ResultSet
...
object JdbcRDD {
  ...
  /**
   * Create an RDD that executes a SQL query on a JDBC connection and reads results. For usage
   * example, see test case JavaAPISuite.testJavaJdbcRDD.
   *
   ...
   * @param mapRow
   *   a function from a ResultSet to a single row of the desired result type(s). This should only
   *   call getInt, getString, etc; the RDD takes care of calling next. The default maps a
   *   ResultSet to an array of Object.
   */
  def create[T](
      sc: JavaSparkContext,
