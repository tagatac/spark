...
class PairRDDFunctions[K, V](self: RDD[(K, V)])(implicit
  ...
  /**
   * Simplified version of combineByKeyWithClassTag that hash-partitions the output RDD. This
   * method is here for backward compatibility. It does not provide combiner classtag information
   * to the shuffle.
   *
   * @see
   *   `combineByKeyWithClassTag`
   ...
   *      U's, as in scala.IterableOnce. The former operation is used for merging values within a
   *      partition, and the latter is used for merging values between partitions. To avoid memory
   *      allocation, both of these functions are allowed to modify and return their first
   *      argument instead of creating a new U.
   */
  ...
  def aggregateByKey[U: ClassTag](
      zeroValue: U)(seqOp: (U, V) => U, combOp: (U, U) => U): RDD[(K, U)] = self.withScope {
   ...
   * return the results immediately to the master as a Map. This will also perform the merging
   ...
   *   rdd.mapValues(_ => 1L).reduceByKey(_ + _), which returns an RDD[T, Long] instead of a map.
   */
  ...
  def countByKeyApprox(
      timeout: Long,
      confidence: Double = 0.95): PartialResult[Map[K, BoundedDouble]] = self.withScope {
   ...
   * partitioning of the resulting key-value pair RDD by passing a Partitioner. The ordering of
   * elements within each group is not guaranteed, and may even differ each time the resulting RDD
   * is evaluated.
   *
   * @note
   ...
   * resulting RDD with into `numPartitions` partitions. The ordering of elements within each
   * group is not guaranteed, and may even differ each time the resulting RDD is evaluated.
   *
   * @note
   *   This operation may be very expensive. If you are grouping in order to perform an
   *   aggregation (such as a sum or average) over each key, using
  ...
  def fullOuterJoin[W](
      other: RDD[(K, W)],
      partitioner: Partitioner): RDD[(K, (Option[V], Option[W]))] = self.withScope {
  ...
  /**
   * Simplified version of combineByKeyWithClassTag that hash-partitions the resulting RDD using
   * the existing partitioner/parallelism level. This method is here for backward compatibility.
  ...
  def cogroup[W1, W2, W3](
      other1: RDD[(K, W1)],
      other2: RDD[(K, W2)],
      other3: RDD[(K, W3)]): RDD[(K, (Iterable[V], Iterable[W1], Iterable[W2], Iterable[W3]))] =
    self.withScope {
  ...
  def cogroup[W1, W2](
      other1: RDD[(K, W1)],
      other2: RDD[(K, W2)],
      numPartitions: Int): RDD[(K, (Iterable[V], Iterable[W1], Iterable[W2]))] = self.withScope {
   ...
   * will be less than or equal to us.
   */
  ...
  /**
   * Output the RDD to any Hadoop-supported file system, using a Hadoop `OutputFormat` class
   * supporting the key and value types K and V in this RDD. Compress the result with the supplied
   * codec.
   */
  ...
  def saveAsNewAPIHadoopFile[F <: NewOutputFormat[K, V]](path: String)(implicit
      fm: ClassTag[F]): Unit = self.withScope {
   ...
   *
   * @note
   *   We should make sure our tasks are idempotent when speculation is enabled, i.e. do not use
   *   output committer that writes data directly. There is an example in
   *   https://issues.apache.org/jira/browse/SPARK-10063 to show the bad result of using direct
   *   output committer with speculation enabled.
   */
  def saveAsHadoopFile(
      path: String,
      keyClass: Class[_],
