...
/**
 * Spark Metrics System, created by a specific "instance", combined by source, sink, periodically
 * polls source metrics data to sink destinations.
 *
 * "instance" specifies "who" (the role) uses the metrics system. In Spark, there are several
 * roles like master, worker, executor, client driver. These roles will create metrics system for
 * monitoring. So, "instance" represents these roles. Currently in Spark, several instances have
 * already implemented: master, worker, executor, driver, applications.
 *
 * "source" specifies "where" (source) to collect metrics data from. In metrics system, there
 * exists two kinds of source:
 *   1. Spark internal source, like MasterSource, WorkerSource, etc, which will collect Spark
 *      component's internal state, these sources are related to instance and will be added after
 *      a specific metrics system is created.
 ...
 *
 * "sink" specifies "where" (destination) to output metrics data to. Several sinks can coexist and
 * metrics can be flushed to all these sinks.
 *
 * Metrics configuration format is like below: [instance].[sink|source].[name].[options] = xxxx
 *
 * [instance] can be "master", "worker", "executor", "driver", "applications" which means only the
 * specified instance has this property. wild card "*" can be used to replace instance name, which
 * means all the instances will have this property.
 *
 * [sink|source] means this property belongs to source or sink. This field can only be source or
 * sink.
 *
...
private[spark] class MetricsSystem private (val instance: String, conf: SparkConf)
  ...
  /**
   * Build a name that uniquely identifies each metric source. The name is structured as follows:
   * <app ID>.<executor ID (or "driver")>.<source name>. If either ID is not available, this
   * defaults to just using <source name>.
   *
   * @param source
   *   Metric source to be named by this method.
   * @return
   *   A unique metric name for each combination of application, executor/driver and metric
   *   source.
   */
