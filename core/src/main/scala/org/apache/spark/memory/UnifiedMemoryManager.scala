 ...
 * this space is further determined by `spark.memory.storageFraction` (default 0.5). This means
 * the size of the storage region is 0.6 * 0.5 = 0.3 of the heap space by default.
 *
 * Storage can borrow as much execution memory as is free until execution reclaims its space. When
 * this happens, cached blocks will be evicted from memory until sufficient borrowed memory is
 * released to satisfy the execution memory request.
 *
 * Similarly, execution can borrow as much storage memory as is free. However, execution memory is
 * *never* evicted by storage due to the complexities involved in implementing this. The
 * implication is that attempts to cache blocks may fail if execution has already eaten up most of
 * the storage space, in which case the new blocks will be evicted immediately according to their
 * respective storage levels.
 *
 * @param onHeapStorageRegionSize
 *   Size of the storage region, in bytes. This region is not statically reserved; execution can
 *   borrow from it if necessary. Cached blocks can be evicted only if actual storage memory usage
 *   exceeds this region.
 */
private[spark] class UnifiedMemoryManager(
   ...
   *
   * Unmanaged memory refers to memory consumed by components that manage their own memory outside
   * of Spark's unified memory management system. Examples include:
   ...
   *
   * The polling mechanism periodically queries registered unmanaged memory consumers to detect
   * inactive consumers and handle cleanup.
   */
   ...
   * @param memoryMode
   *   The memory mode (ON_HEAP or OFF_HEAP) to get usage for
   * @return
   *   The current unmanaged memory usage in bytes
   */
  ...
  override private[memory] def acquireExecutionMemory(
     ...
     *
     * When acquiring memory for a task, the execution pool may need to make multiple attempts.
     * Each attempt must be able to evict storage in case another task jumps in and caches a large
     * block between the attempts. This is called once per attempt.
     */
     ...
     *
     * The execution memory pool divides this quantity among the active tasks evenly to cap the
     * execution memory allocation for each task. It is important to keep this greater than the
     * execution pool size, which doesn't take into account potential memory that could be freed
     * by evicting storage. Otherwise we may hit SPARK-12155.
     *
     * Additionally, this quantity should be kept below `maxMemory` to arbitrate fairness in
     * execution memory allocation across tasks, Otherwise, a task may occupy more than its fair
     * share of execution memory, mistakenly thinking that other tasks can acquire the portion of
     * storage memory that cannot be evicted.
     *
     * This also factors in unmanaged memory usage to ensure we don't over-allocate memory when
     * unmanaged components are consuming significant memory.
     */
...
object UnifiedMemoryManager extends Logging {
  ...
  /**
   * Returns the total unmanaged memory in bytes, including both on-heap unmanaged memory and
   * off-heap unmanaged memory.
   */
  private[spark] def getUnmanagedMemoryUsed: Long = {
    UnifiedMemoryManager.unmanagedOnHeapUsed.get() + UnifiedMemoryManager.unmanagedOffHeapUsed
      .get()
  }
   ...
   *
   * Unmanaged memory consumers are components that manage their own memory outside of Spark's
   * unified memory management system. By registering, their memory usage will be periodically
   * polled and factored into Spark's memory allocation decisions.
   *
   * @param unmanagedMemoryConsumer
   *   The consumer to register for memory tracking
   */
  def registerUnmanagedMemoryConsumer(unmanagedMemoryConsumer: UnmanagedMemoryConsumer): Unit = {
    val id = unmanagedMemoryConsumer.unmanagedMemoryConsumerId
    unmanagedMemoryConsumers.put(id, unmanagedMemoryConsumer)
  }

  /**
   * Unregister an unmanaged memory consumer. This should be called when a component is shutting
   * down to prevent memory leaks and ensure accurate memory tracking.
   *
   * @param unmanagedMemoryConsumer
   *   The consumer to unregister. Only used in tests
   */
   ...
   * @param componentType
   *   The type of component to filter by (e.g., "RocksDB")
   * @return
   *   Total memory usage in bytes for the specified component type
   */
  ...
  /**
   * Clear all unmanaged memory users. This is useful during executor shutdown or cleanup. Since
   * each executor runs in its own JVM, this clears all users for this executor.
   */
  ...
  /**
   * Start unmanaged memory polling if not already started. This ensures only one polling thread
   * is created per JVM, regardless of how many UnifiedMemoryManager instances are created.
   */
  private[memory] def startPollingIfNeeded(pollingIntervalMs: Long): Unit = {
    if (pollingStarted.compareAndSet(false, true)) {
      unmanagedMemoryPoller =
        ThreadUtils.newDaemonSingleThreadScheduledExecutor("unmanaged-memory-poller")
      ...
      logInfo(
        log"Unmanaged memory polling started with interval " +
  ...
  private def pollUnmanagedMemoryUsers(): Unit = {
    ...
    val memoryUsages = consumers.map { case (userId, memoryUser) =>
      try {
        ...
        if (memoryUsed == -1L) {
          logDebug(
            log"Unmanaged memory consumer ${MDC(LogKeys.OBJECT_ID, userId.toString)} " +
              log"is no longer active, marking for removal")
          (userId, memoryUser, None) // Mark for removal
        } else if (memoryUsed < 0L) {
          logWarning(
            log"Invalid memory usage value ${MDC(LogKeys.NUM_BYTES, memoryUsed)} " +
        ...
        case NonFatal(e) =>
          logWarning(
            log"Failed to get memory usage for unmanaged memory user " +
    ...
    memoryUsages.filter(_._3.isEmpty).foreach { case (userId, _, _) =>
      ...
      logInfo(
        log"Removed inactive unmanaged memory consumer " +
    ...
    logDebug(
      s"Unmanaged memory polling completed: $numConsumers consumers, " +
