 ...
 */
private[spark] object SparkHadoopWriter extends Logging {
  ...
  def write[K, V: ClassTag](rdd: RDD[(K, V)], config: HadoopWriteConfigUtil[K, V]): Unit = {
    ...
    try {
      ...
      logInfo(
        log"Write Job ${MDC(JOB_ID, jobContext.getJobID)} committed." +
...
/**
 * A helper class that reads JobConf from older mapred API, creates output
 ...
 * Format/Committer/Writer.
 */
private[spark] class HadoopMapReduceWriteConfigUtil[K, V: ClassTag](
