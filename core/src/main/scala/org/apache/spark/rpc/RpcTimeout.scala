 ...
 * @param timeoutProp
 *   the configuration property that controls this timeout
 */
private[spark] class RpcTimeout(val duration: FiniteDuration, val timeoutProp: String)
   ...
   *
   * @note
   *   This can be used in the recover callback of a Future to add to a TimeoutException Example:
   *   val timeout = new RpcTimeout(5.milliseconds, "short timeout") Future(throw new
   ...
   * timeout, throw a [[RpcTimeoutException]] to indicate which configuration controls the
   * timeout.
   *
   * @param future
   *   the `Future` to be awaited
   * @throws RpcTimeoutException
   *   if after waiting for the specified time `future` is still not ready
   */
...
private[spark] object RpcTimeout {
  ...
  /**
   * Lookup the timeout property in the configuration and create a RpcTimeout with the property
   * key in the description.
   *
   * @param conf
   *   configuration properties containing the timeout
   * @param timeoutProp
   *   property key for the timeout in seconds
  ...
  /**
   * Lookup the timeout property in the configuration and create a RpcTimeout with the property
   * key in the description. Uses the given default value if property is not set
   *
   * @param conf
   *   configuration properties containing the timeout
   * @param timeoutProp
   *   property key for the timeout in seconds
   * @param defaultValue
   *   default timeout value in seconds if property not found
   */
  ...
  /**
   * Lookup prioritized list of timeout properties in the configuration and create a RpcTimeout
   * with the first set property key in the description. Uses the given default value if property
   * is not set
   *
   * @param conf
   *   configuration properties containing the timeout
   * @param timeoutPropList
   *   prioritized list of property keys for the timeout in seconds
   * @param defaultValue
   *   default timeout value in seconds if no properties found
   */
