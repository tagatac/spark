...
trait RDDCheckpointTester { self: SparkFunSuite =>
   ...
   * serialized size of the RDD is reduce after checkpointing or not. This function should be
   * called on all RDDs that have a parent RDD (i.e., do not call on ParallelCollection, BlockRDD,
   * etc.).
   *
   * @param op
   *   an operation to run on the RDD
   * @param reliableCheckpoint
   *   if true, use reliable checkpoints, otherwise use local checkpoints
   * @param collectFunc
   *   a function for collecting the values in the RDD, in case there are non-comparable types
   *   like arrays that we want to convert to something that supports ==
   */
  ...
  /**
   * Test whether checkpointing of the parent of the generated RDD also truncates the lineage or
   * not. Some RDDs like CoGroupedRDD hold on to its parent RDDs partitions. So even if the parent
   * RDD is checkpointed and its partitions changed, the generated RDD will remember the
   * partitions and therefore potentially the whole lineage. This function should be called only
   * those RDD whose partitions refer to parent RDD's partitions (i.e., do not call it on simple
   * RDDs).
   *
   * @param op
   *   an operation to run on the RDD
  ...
  /**
   * Serialize and deserialize an object. This is useful to verify the objects contents after
   * deserialization (e.g., the contents of an RDD split after it is sent to an executor along
   * with a task)
   */
  ...
  /**
   * Generate an pair RDD (with partitioner) such that both the RDD and its partitions have large
   * size.
   */
...
/**
 * Test suite for end-to-end checkpointing functionality. This tests both reliable checkpoints and
 * local checkpoints.
 */
    ...
    testRDD(
      rdd => {
        new PartitionerAwareUnionRDD[(Int, Int)](
          ...
          Array(
            generateFatPairRDD(),
            rdd.map(x => (x % 2, 1)).reduceByKey(partitioner, _ + _)).toImmutableArraySeq)
      },
      reliableCheckpoint)
