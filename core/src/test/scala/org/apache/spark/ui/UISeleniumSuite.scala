...
class UISeleniumSuite extends SparkFunSuite with WebBrowser with Matchers {
  ...
  test("effects of unpersist() / persist() should be reflected") {
    ...
    withSpark(newSparkContext()) { sc =>
      ...
      eventually(timeout(5.seconds), interval(50.milliseconds)) {
        ...
        val tableRowText =
          findAll(cssSelector("#rdd-storage-by-block-table td")).map(_.text).toSeq
        tableRowText should contain(StorageLevels.DISK_ONLY.description)
      }
    ...
    withSpark(newSparkContext()) { sc =>
      ...
      val mappedData = data.map { x =>
        ...
        if (taskContext.taskAttemptId() == 1) {
          ...
          throw new FetchFailedException(bmAddress, shuffleId, mapId, mapIndex, reduceId, message)
        } else {
          x
        }
      }
  ...
  test("kill stage POST/GET response is correct") {
    withSpark(newSparkContext(killEnabled = true)) { sc =>
      sc.parallelize(1 to 10).map { x => Thread.sleep(10000); x }.countAsync()
      eventually(timeout(5.seconds), interval(50.milliseconds)) {
        val url = new URI(sc.ui.get.webUrl.stripSuffix("/") + "/stages/stage/kill/?id=0").toURL
  ...
  test("kill job POST/GET response is correct") {
    withSpark(newSparkContext(killEnabled = true)) { sc =>
      sc.parallelize(1 to 10).map { x => Thread.sleep(10000); x }.countAsync()
      eventually(timeout(5.seconds), interval(50.milliseconds)) {
        val url = new URI(sc.ui.get.webUrl.stripSuffix("/") + "/jobs/job/kill/?id=0").toURL
        // SPARK-6846: should be POST only but YARN AM doesn't proxy POST
        TestUtils.httpResponseCode(url, "GET") should be(200)
        TestUtils.httpResponseCode(url, "POST") should be(200)
      }
  ...
  test("stage & job retention") {
    ...
    withSpark(sc) { sc =>
      ...
      val expStageInfo = Seq(("19", "collect"), ("18", "count"), ("17", "groupBy"))
  ...
  test("live UI json application list") {
    withSpark(newSparkContext()) { sc =>
      val appListRawJson =
        HistoryServerSuite.getUrl(new URI(sc.ui.get.webUrl + "/api/v1/applications").toURL)
      ...
      val durationAdjusted = oneAppJsonAst.transformField { case ("duration", _) =>
        ("duration", duration)
      }
    ...
    withSpark(newSparkContext()) { sc =>
      ...
      eventually(timeout(5.seconds), interval(100.milliseconds)) {
        val stage0 = Utils.tryWithResource(
          Source.fromURL(sc.ui.get.webUrl +
    ...
    Seq(true, false).foreach { timelineEnabled =>
      withSpark(newSparkContext(timelineEnabled = timelineEnabled)) { sc =>
        ...
        eventually(timeout(10.seconds), interval(50.milliseconds)) {
          ...
          assert(
            findAll(className("expand-task-assignment-timeline")).nonEmpty === timelineEnabled)
        }
      }
    }
  }
    ...
    withSpark(newSparkContext()) { sc =>
      ...
      eventually(timeout(5.seconds), interval(50.milliseconds)) {
        ...
        val encodeOnceRes = Utils.tryWithResource(
          Source.fromURL(apiUrl(sc.ui.get, "stages/0/0/taskTable?" + encodeOnceQuery)))(
          _.mkString)
        val encodeTwiceRes = Utils.tryWithResource(
          Source.fromURL(apiUrl(sc.ui.get, "stages/0/0/taskTable?" + encodeTwiceQuery)))(
          _.mkString)
