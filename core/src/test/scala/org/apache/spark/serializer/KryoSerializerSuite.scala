...
class KryoSerializerSuite extends SparkFunSuite with SharedSparkContext {
  ...
  test("SPARK-7392 configuration limits") {
    ...
    val thrown2 =
      intercept[IllegalArgumentException](newKryoInstance(conf, maxBufferSize = "2048m"))
  ...
  test("kryo with parallelize") {
    val control = 1 :: 2 :: Nil
    val result =
      sc.parallelize(control.map(new ClassWithoutNoArgConstructor(_))).map(_.x).collect()
    assert(control === result.toSeq)
  }
  ...
  test("registration of HighlyCompressedMapStatus") {
    ...
    Seq(denseBlockSizes, sparseBlockSizes).foreach { blockSizes =>
      mapTaskId += 1
      ser.serialize(
        HighlyCompressedMapStatus(BlockManagerId("exec-1", "host", 1234), blockSizes, mapTaskId))
    }
  }
  ...
  test("getAutoReset") {
    ...
    val conf = new SparkConf()
      .set(KRYO_USER_REGISTRATORS, Seq(classOf[RegistratorWithoutAutoReset].getName))
...
class KryoSerializerAutoResetDisabledSuite extends SparkFunSuite with SharedSparkContext {
  ...
  test("sort-shuffle with bypassMergeSort (SPARK-7873)") {
    ...
    assert(
      sc.parallelize(Seq.fill(100)(myObject)).repartition(2).collect().toSet === Set(myObject))
  }
