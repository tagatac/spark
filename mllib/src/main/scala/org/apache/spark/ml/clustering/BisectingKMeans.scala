...
private[clustering] trait BisectingKMeansParams
  ...
  /**
   * The desired number of leaf clusters. Must be &gt; 1. Default: 4. The actual number could be
  ...
  /**
   * The minimum number of points (if greater than or equal to 1.0) or the minimum proportion of
   * points (if less than 1.0) of a divisible cluster (default: 1.0).
   * @group expertParam
   */
  @Since("2.0.0")
 ...
 *
 * @param parentModel
 *   a model trained by [[org.apache.spark.mllib.clustering.BisectingKMeans]].
 */
@Since("2.0.0")
class BisectingKMeansModel private[ml] (
  ...
  /**
   * Computes the sum of squared distances between the input points and their corresponding
   * cluster centers.
   *
   * @deprecated
   *   This method is deprecated and will be removed in future versions. Use ClusteringEvaluator
 ...
 *
 * @param predictions
 *   `DataFrame` produced by `BisectingKMeansModel.transform()`.
 * @param predictionCol
 ...
 * @param trainingCost
 *   Sum of the cost to the nearest centroid for all points in the training dataset. This is
