 ...
 *
 * @param weightedNumExamples
 *   Weighted count of samples in the tree.
 * @param numClasses
 *   For classification: labels can take values {0, ..., numClasses - 1}. For regression: fixed at
 *   0 (no meaning).
 * @param maxBins
 *   Maximum number of bins, for all features.
 * @param featureArity
 *   Map: categorical feature index to arity. I.e., the feature takes values in {0, ..., arity -
 *   1}.
 * @param numBins
 *   Number of bins for each feature.
 * @param minWeightFractionPerNode
 *   The minimum fraction of the total sample weight that must be present in a leaf node in order
 *   to be considered a valid split.
 */
private[spark] class DecisionTreeMetadata(
  ...
  /**
   * Number of splits for the given feature. For unordered features, there is 1 bin per split. For
   * ordered features, there is 1 more bin than split.
   */
  ...
  /**
   * Set number of splits for a continuous feature. For a continuous feature, number of bins is
   * number of splits plus 1.
   */
...
private[spark] object DecisionTreeMetadata extends Logging {
  ...
  /**
   * Construct a [[DecisionTreeMetadata]] instance for this dataset and parameters. This computes
   * which categorical features will be ordered vs. unordered, as well as the number of splits and
  ...
  def buildMetadata(
    ...
    require(
      numFeatures > 0,
      s"DecisionTree requires number of features > 0, " +
    ...
    if (maxPossibleBins < strategy.maxBins) {
      logWarning(
        log"DecisionTree reducing maxBins from " +
    ...
    new DecisionTreeMetadata(
      ...
      strategy.minWeightFractionPerNode,
   ...
   */
  def buildMetadata(input: RDD[Instance], strategy: Strategy): DecisionTreeMetadata = {
    buildMetadata(input, strategy, numTrees = 1, featureSubsetStrategy = "all")
  }

  /**
   * Given the arity of a categorical feature (arity = number of categories), return the number of
   * bins for the feature if it is to be treated as an unordered feature. There is 1 split for
   * every partitioning of categories into 2 disjoint, non-empty sets; there are math.pow(2, arity -
   * 1) - 1 such splits. Each split has 2 corresponding bins.
   */
