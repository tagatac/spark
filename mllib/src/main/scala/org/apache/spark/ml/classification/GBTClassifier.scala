...
/**
 * Gradient-Boosted Trees (GBTs) (http://en.wikipedia.org/wiki/Gradient_boosting) learning
 * algorithm for classification. It supports binary labels, as well as both continuous and
 ...
 *   - TreeBoost (Friedman, 1999) additionally modifies the outputs at tree leaf nodes based on
 *     the loss function, whereas the original gradient boosting method does not.
 ...
 */
@Since("1.4.0")
class GBTClassifier @Since("1.4.0") (@Since("1.4.0") override val uid: String)
  ...
  /**
   * The impurity setting is ignored for GBT models. Individual trees are built using impurity
   * "Variance."
   *
   * @group setParam
   */
  ...
  override protected def train(dataset: Dataset[_]): GBTClassificationModel = instrumented {
    instr =>
...
/**
 * Gradient-Boosted Trees (GBTs) (http://en.wikipedia.org/wiki/Gradient_boosting) model for
 * classification. It supports binary labels, as well as both continuous and categorical features.
 *
 * @param _trees
...
class GBTClassificationModel private[ml] (
  ...
  // For ml connect only
  private[ml] def this() =
    this("", Array(new DecisionTreeRegressionModel), Array(Double.NaN), -1, -1)
    ...
    rawPrediction match {
      ...
      case sv: SparseVector =>
        throw new RuntimeException(
          "Unexpected error in GBTClassificationModel:" +
  ...
  override def copy(extra: ParamMap): GBTClassificationModel = {
    copyValues(
      new GBTClassificationModel(uid, _trees, _treeWeights, numFeatures, numClasses),
...
object GBTClassificationModel extends MLReadable[GBTClassificationModel] {
  ...
  private[GBTClassificationModel] class GBTClassificationModelWriter(
      instance: GBTClassificationModel)
      extends MLWriter {

    override protected def saveImpl(path: String): Unit = {

      val extraMetadata: JObject =
        Map(numFeaturesKey -> instance.numFeatures, numTreesKey -> instance.getNumTrees)
  ...
  private class GBTClassificationModelReader extends MLReader[GBTClassificationModel] {
    ...
    override def load(path: String): GBTClassificationModel = {
      ...
      require(
        ...
        s"GBTClassificationModel.load expected $numTrees" +
