 ...
 *   number of clusters
 * @param assignments
 *   an RDD of clustering `PowerIterationClustering#Assignment`s
 */
@Since("1.3.0")
...
object PowerIterationClusteringModel extends Loader[PowerIterationClusteringModel] {
  ...
  private[clustering] object SaveLoadV1_0 {
    ...
    def load(sc: SparkContext, path: String): PowerIterationClusteringModel = {
      ...
      val assignmentsRDD = assignments.rdd.map { case Row(id: Long, cluster: Int) =>
...
/**
 * Power Iteration Clustering (PIC), a scalable graph clustering algorithm developed by <a
 * href="http://www.cs.cmu.edu/~frank/papers/icml2010-pic-final.pdf">Lin and Cohen</a>. From the
 ...
 * @param initMode
 *   Set the initialization mode. This can be either "random" to use a random vector as vertex
 *   properties, or "degree" to use normalized sum similarities. Default: random.
 *
 * @see
 *   <a href="http://en.wikipedia.org/wiki/Spectral_clustering"> Spectral clustering
 *   (Wikipedia)</a>
 */
@Since("1.3.0")
class PowerIterationClustering private[clustering] (
  ...
  /**
   * Constructs a PIC instance with default parameters: {k: 2, maxIterations: 100, initMode:
   * "random"}.
   */
  @Since("1.3.0")
   ...
   *
   * @param graph
   *   an affinity matrix represented as graph, which is the matrix A in the PIC paper. The
   *   similarity s,,ij,, represented as the edge between vertices (i, j) must be nonnegative.
   *   This is a symmetric matrix and hence s,,ij,, = s,,ji,,. For any (i, j) with nonzero
   *   similarity, there should be either (i, j, s,,ij,,) or (j, i, s,,ji,,) in the input. Tuples
   *   with i = j are ignored, because we assume s,,ij,, = 0.0.
   *
   * @return
   ...
   *
   * @param similarities
   *   an RDD of (i, j, s,,ij,,) tuples representing the affinity matrix, which is the matrix A in
   *   the PIC paper. The similarity s,,ij,, must be nonnegative. This is a symmetric matrix and
   *   hence s,,ij,, = s,,ji,,. For any (i, j) with nonzero similarity, there should be either (i,
   *   j, s,,ij,,) or (j, i, s,,ji,,) in the input. Tuples with i = j are ignored, because we
   *   assume s,,ij,, = 0.0.
   *
   * @return
   *   a [[PowerIterationClusteringModel]] that contains the clustering result
   ...
   *
   * @param w
   *   The normalized affinity matrix, which is the matrix W in the PIC paper with w,,ij,, =
   *   a,,ij,, / d,,ii,, as its edge properties and the initial vector of the power iteration as
   *   its vertex properties.
   */
  private def pic(w: Graph[Double, Double]): PowerIterationClusteringModel = {
    val v = powerIter(w, maxIterations)
    val assignments = kMeans(v, k).map { case (id, cluster) =>
...
object PowerIterationClustering extends Logging {
   ...
   */
  private[clustering] def normalize(graph: Graph[Double, Double]): Graph[Double, Double] = {
  ...
  /**
   * Normalizes the affinity matrix (A) by row sums and returns the normalized affinity matrix
   * (W).
   */
   ...
   *
   * @param g
   *   a graph representing the normalized affinity matrix (W)
   * @return
   *   a graph with edges representing W and vertices representing a random vector with unit
   *   1-norm
   */
  private[clustering] def randomInit(g: Graph[Double, Double]): Graph[Double, Double] = {
    val r = g.vertices
  ...
  /**
   * Generates the degree vector as the vertex properties (v0) to start power iteration. It is not
   * exactly the node degrees but just the normalized sum similarities. Call it as degree vector
   * because it is used in the PIC paper.
   *
   * @param g
   *   a graph representing the normalized affinity matrix (W)
   * @return
   ...
   */
  private[clustering] def initDegreeVector(g: Graph[Double, Double]): Graph[Double, Double] = {
  ...
  private[clustering] def powerIter(
    ...
    for (iter <- 0 until maxIterations if math.abs(diffDelta) > tol) {
      ...
      if (math.abs(diffDelta) < tol) {
         ...
         * but with opposite sign. The resultant vector flip-flops between two vectors. We should
         * give an exception, if we detect the failure of the convergence of the power iteration
         */

        // Rayleigh quotient = x^tAx / x^tx
        val xTAx = curG
          .joinVertices(v) { case (_, x, y) =>
            x * y
          }
          .vertices
          .values
