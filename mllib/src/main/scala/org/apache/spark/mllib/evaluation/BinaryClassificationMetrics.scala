 ...
 * @param numBins
 *   if greater than 0, then the curves (ROC curve, PR curve) computed internally will be
 *   down-sampled to this many "bins". If 0, no down-sampling will occur. This is useful because
 *   the curve contains a point for each distinct score in the input, and this could be as large
 *   as the input itself -- millions of points or more, when thousands may be entirely sufficient
 *   to summarize the curve. After down-sampling, the curves will instead be made of approximately
 *   `numBins` points instead. Points are made from bins of equal numbers of consecutive points.
 *   The size of each bin is `floor(scoreAndLabels.count() / numBins)`, which means the resulting
 *   number of bins may not exactly equal numBins. The last bin in each partition may be smaller
 *   as a result, meaning there may be an extra sample at partition boundaries.
 */
@Since("1.0.0")
class BinaryClassificationMetrics @Since("3.0.0") (
   ...
   * @param scoreAndLabels
   *   a DataFrame with two double columns: score and label
   */
  ...
  /**
   * Returns the receiver operating characteristic (ROC) curve, which is an RDD of (false positive
   * rate, true positive rate) with (0.0, 0.0) prepended and (1.0, 1.0) appended to it.
   * @see
   *   <a href="http://en.wikipedia.org/wiki/Receiver_operating_characteristic"> Receiver
   *   operating characteristic (Wikipedia)</a>
   */
  @Since("1.0.0")
  ...
  /**
   * Returns the precision-recall curve, which is an RDD of (recall, precision), NOT (precision,
   * recall), with (0.0, p) prepended to it, where p is the precision associated with the lowest
   * recall on the curve.
   * @see
   *   <a href="http://en.wikipedia.org/wiki/Precision_and_recall"> Precision and recall
   *   (Wikipedia)</a>
   */
  @Since("1.0.0")
   ...
   * @see
   *   <a href="http://en.wikipedia.org/wiki/F1_score">F1 score (Wikipedia)</a>
   */
  @Since("1.0.0")
    ...
    val binnedCounts =
      ...
      if (numBins == 0) {
        ...
        if (grouping < 2) {
          ...
          logInfo(
            log"Curve is too small (${MDC(COUNT, countsSize)}) " +
