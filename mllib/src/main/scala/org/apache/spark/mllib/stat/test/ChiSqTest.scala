...
/**
 * Conduct the chi-squared test for the input RDDs using the specified method. Goodness-of-fit
 * test is conducted on two `Vectors`, whereas test of independence is conducted on an input of
 * type `Matrix` in which independence between columns is assessed. We also provide a method for
 * computing the chi-squared statistic between each feature and the label for an input
 * `RDD[LabeledPoint]`, return an `Array[ChiSquaredTestResult]` of size = number of features in
 * the input RDD.
 *
 * Supported methods for goodness of fit: `pearson` (default) Supported methods for independence:
 * `pearson` (default)
 *
...
private[spark] object ChiSqTest extends Logging {
  ...
  /**
   * @param name
   *   String name for the method.
   * @param chiSqFunc
   *   Function for computing the statistic given the observed and expected counts.
   */
  ...
  private def computeChiSq(
    ...
    val contingency =
      new DenseMatrix(numValues, numLabels, Array.ofDim[Double](numValues * numLabels))
  ...
  def chiSquared(
    ...
    if (size > 1000) {
      logWarning(
        log"Chi-squared approximation may not be accurate due to low expected " +
