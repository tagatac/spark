 ...
 *   Number of columns.
 * @param rowsPerPart
 *   Number of rows per partition, which may be less at the bottom edge.
 * @param colsPerPart
 *   Number of columns per partition, which may be less at the right edge.
 */
private[mllib] class GridPartitioner(
   ...
   *
   * @param key
   *   The partition id i (calculated through this method for coordinate (i, j) in
   *   `simulateMultiply`, the coordinate (i, j) or a tuple (i, j, k), where k is the inner index
   *   used in multiplication. k is ignored in computing partitions.
   * @return
   *   The index of the partition, which the coordinate belongs to.
   */
 ...
 *   distributed matrix. If multiple blocks with the same index exist, the results for operations
 *   like add and multiply will be unpredictable.
 * @param rowsPerBlock
 *   Number of rows that make up each block. The blocks forming the final rows are not required to
 *   have the given number of rows
 * @param colsPerBlock
 *   Number of columns that make up each block. The blocks forming the final columns are not
 *   required to have the given number of columns
 * @param nRows
 *   Number of rows of this matrix. If the supplied value is less than or equal to zero, the
 *   number of rows will be calculated when `numRows` is invoked.
 * @param nCols
 *   Number of columns of this matrix. If the supplied value is less than or equal to zero, the
 *   number of columns will be calculated when `numCols` is invoked.
 */
@Since("1.3.0")
class BlockMatrix @Since("1.3.0") (
   ...
   * @param blocks
   *   The RDD of sub-matrix blocks ((blockRowIndex, blockColIndex), sub-matrix) that form this
   *   distributed matrix. If multiple blocks with the same index exist, the results for
   *   operations like add and multiply will be unpredictable.
   * @param rowsPerBlock
   *   Number of rows that make up each block. The blocks forming the final rows are not required
   *   to have the given number of rows
   * @param colsPerBlock
   *   Number of columns that make up each block. The blocks forming the final columns are not
   *   required to have the given number of columns
   */
  ...
  private def estimateDim(): Unit = {
    val (rows, cols) = blockInfo
      .map { case ((blockRowIndex, blockColIndex), (m, n)) =>
  ...
  def toIndexedRowMatrix(): IndexedRowMatrix = {
    ...
    require(
      cols < Int.MaxValue,
      s"The number of columns should be less than Int.MaxValue ($cols).")

    val rows = blocks
      .flatMap { case ((blockRowIdx, blockColIdx), mat) =>
        mat.rowIter.zipWithIndex.filter(_._1.size > 0).map { case (vector, rowIdx) =>
  ...
  def toLocalMatrix(): Matrix = {
    ...
    if (mem > 500) {
      logWarning(
        log"Storing this matrix will require ${MDC(LogKeys.MEMORY_SIZE, mem)} " +
          log"MiB of memory!")
    }
  ...
  /**
   * Transpose this `BlockMatrix`. Returns a new `BlockMatrix` instance sharing the same
   * underlying data. Is a lazy operation.
   */
  @Since("1.3.0")
  ...
  /**
   * For given matrices `this` and `other` of compatible dimensions and compatible block
   * dimensions, it applies a binary function on their corresponding blocks.
   *
   * @param other
   *   The second BlockMatrix argument for the operator specified by `binMap`
   * @param binMap
   *   A function taking two breeze matrices and returning a breeze matrix
   * @return
   *   A [[BlockMatrix]] whose blocks are the results of a specified binary map on blocks of
   *   `this` and `other`. Note: `blockMap` ONLY works for `add` and `subtract` methods and it
   *   does not support operators such as (a, b) => -a + b TODO: Make the use of zero matrices
  ...
  /**
   * Adds the given block matrix `other` to `this` block matrix: `this + other`. The matrices must
   * have the same size and matching `rowsPerBlock` and `colsPerBlock` values. If one of the
   * blocks that are being added are instances of `SparseMatrix`, the resulting sub matrix will
   * also be a `SparseMatrix`, even if it is being added to a `DenseMatrix`. If two dense matrices
   * are added, the output will also be a `DenseMatrix`.
   */
  @Since("1.3.0")
  ...
  /**
   * Subtracts the given block matrix `other` from `this` block matrix: `this - other`. The
   * matrices must have the same size and matching `rowsPerBlock` and `colsPerBlock` values. If
   * one of the blocks that are being subtracted are instances of `SparseMatrix`, the resulting
   * sub matrix will also be a `SparseMatrix`, even if it is being subtracted from a
   * `DenseMatrix`. If two dense matrices are subtracted, the output will also be a `DenseMatrix`.
   */
  @Since("2.0.0")
   ...
   * when we are actually shuffling the matrices. The `colsPerBlock` of this matrix must equal the
   * `rowsPerBlock` of `other`. Exposed for tests.
   *
   * @param other
   *   The BlockMatrix to multiply
   * @param partitioner
   *   The partitioner that will be used for the resulting matrix `C = A * B`
   * @return
   *   A tuple of [[BlockDestinations]]. The first element is the Map of the set of partitions
   ...
   * of this matrix must equal the `rowsPerBlock` of `other`. If `other` contains `SparseMatrix`,
   * they will have to be converted to a `DenseMatrix`. The output [[BlockMatrix]] will only
   * consist of blocks of `DenseMatrix`. This may cause some performance issues until support for
   * multiplying two sparse matrices is added.
   *
   * @note
   *   The behavior of multiply has changed in 1.6.0. `multiply` used to throw an error when there
   *   were blocks with duplicate indices. Now, the blocks with duplicate indices will be added
   *   with each other.
   */
  @Since("1.3.0")
   ...
   * they will have to be converted to a `DenseMatrix`. The output [[BlockMatrix]] will only
   * consist of blocks of `DenseMatrix`. This may cause some performance issues until support for
   * multiplying two sparse matrices is added. Blocks with duplicate indices will be added with
   * each other.
   *
   * @param other
   *   Matrix `B` in `A * B = C`
   * @param numMidDimSplits
   *   Number of splits to cut on the middle dimension when doing multiplication. For example,
   *   when multiplying a Matrix `A` of size `m x n` with Matrix `B` of size `n x k`, this
   *   parameter configures the parallelism to use when grouping the matrices. The parallelism
   *   will increase from `m x k` to `m x k x numMidDimSplits`, which in some cases also reduces
   *   total shuffled data.
   */
  @Since("2.2.0")
  def multiply(other: BlockMatrix, numMidDimSplits: Int): BlockMatrix = {
    ...
    if (colsPerBlock == other.rowsPerBlock) {
      ...
      val newBlocks = flatA
        .cogroup(flatB, intermediatePartitioner)
