...
trait MLTest extends StreamTest with TempDirectory { self: Suite =>
  ...
  private[util] def testTransformerOnStreamData[A: Encoder](
      ...
      otherResultCols: String*)(globalCheckFunction: Seq[Row] => Unit): Unit = {
    ...
    val streamDF = stream
      .toDS()
      .toDF(columnNames.toImmutableArraySeq: _*)
      .select(columnsWithMetadata: _*)
    val data = dataframe.as[A].collect()

    val streamOutput = transformer
      .transform(streamDF)
  ...
  def testTransformer[A: Encoder](
      ...
      transformer: Transformer,
      firstResultCol: String,
      otherResultCols: String*)(checkFunction: Row => Unit): Unit = {
  ...
  def testTransformerByGlobalCheckFunc[A: Encoder](
      ...
      firstResultCol: String,
      otherResultCols: String*)(globalCheckFunction: Seq[Row] => Unit): Unit = {
    testTransformerOnStreamData(dataframe, transformer, firstResultCol, otherResultCols: _*)(
      globalCheckFunction)
    testTransformerOnDF(dataframe, transformer, firstResultCol, otherResultCols: _*)(
      globalCheckFunction)
  }
  ...
  def testPredictionModelSinglePrediction(
      model: PredictionModel[Vector, _],
      dataset: Dataset[_]): Unit = {
    model
      .transform(dataset)
      .select(model.getFeaturesCol, model.getPredictionCol)
      .collect()
  ...
  def testClassificationModelSingleRawPrediction(
      model: ClassificationModel[Vector, _],
      dataset: Dataset[_]): Unit = {
    model
      .transform(dataset)
      .select(model.getFeaturesCol, model.getRawPredictionCol)
      .collect()
      .foreach { case Row(features: Vector, rawPrediction: Vector) =>
  ...
  def testProbClassificationModelSingleProbPrediction(
    ...
    model
      .transform(dataset)
      .select(model.getFeaturesCol, model.getProbabilityCol)
      .collect()
      .foreach { case Row(features: Vector, probPrediction: Vector) =>
  ...
  def testClusteringModelSinglePrediction(
    ...
    model
      ...
      .select(input, output)
      .collect()
      .foreach { case Row(features: Vector, prediction: Int) =>
  ...
  def testClusteringModelSingleProbabilisticPrediction(
    ...
    model
      ...
      .select(input, output)
      .collect()
      .foreach { case Row(features: Vector, prediction: Vector) =>
        assert(prediction === transform(features))
      }
  }
  ...
  def testInvalidRegressionLabels(f: DataFrame => Any): Unit = {
    ...
    val df1 = sc
      .parallelize(
        Seq((null, 1.0, Vectors.dense(1.0, 2.0)), ("1.0", 1.0, Vectors.dense(1.0, 2.0))))
    ...
    val df2 = sc
      .parallelize(
        Seq((Double.NaN, 1.0, Vectors.dense(1.0, 2.0)), (1.0, 1.0, Vectors.dense(1.0, 2.0))))
  ...
  def testInvalidClassificationLabels(f: DataFrame => Any, numClasses: Option[Int]): Unit = {
    ...
    numClasses match {
      case Some(2) =>
        ...
        val df3 = sc
          .parallelize(
            Seq((3.0, 1.0, Vectors.dense(1.0, 2.0)), (1.0, 1.0, Vectors.dense(1.0, 2.0))))
          .toDF("label", "weight", "features")
        val e3 = intercept[Exception](f(df3))
      ...
      case _ =>
        ...
        val df3 = sc
          .parallelize(
            Seq((1.0, 1.0, Vectors.dense(1.0, 2.0)), (-3.0, 1.0, Vectors.dense(1.0, 2.0))))
          .toDF("label", "weight", "features")
        val e3 = intercept[Exception](f(df3))
        ...
        val df5 = sc
          .parallelize(
            Seq((1.0, 1.0, Vectors.dense(1.0, 2.0)), (0.1, 1.0, Vectors.dense(1.0, 2.0))))
          .toDF("label", "weight", "features")
  ...
  def testInvalidWeights(f: DataFrame => Any): Unit = {
    ...
    val df1 = sc
      .parallelize(
        Seq((1.0, "1.0", Vectors.dense(1.0, 2.0)), (0.0, null, Vectors.dense(1.0, 2.0))))
    ...
    val df2 = sc
      .parallelize(
        Seq((1.0, 1.0, Vectors.dense(1.0, 2.0)), (0.0, Double.NaN, Vectors.dense(1.0, 2.0))))
      .toDF("label", "weight", "features")
    val e2 = intercept[Exception](f(df2))
    ...
    val df3 = sc
      .parallelize(Seq((1.0, 1.0, Vectors.dense(1.0, 2.0)), (0.0, -3.0, Vectors.dense(1.0, 2.0))))
      .toDF("label", "weight", "features")
    val e3 = intercept[Exception](f(df3))
  ...
  def testInvalidVectors(f: DataFrame => Any): Unit = {
    ...
    val df1 = sc
      .parallelize(Seq((1.0, 1.0, Vectors.dense(1.0, 2.0)), (0.0, 1.0, null)))
      .toDF("label", "weight", "features")
    val e1 = intercept[Exception](f(df1))
    ...
    val df2 = sc
      .parallelize(
        Seq((1.0, 1.0, Vectors.dense(1.0, 2.0)), (0.0, 1.0, Vectors.dense(Double.NaN, 2.0))))
      .toDF("label", "weight", "features")
    val e2 = intercept[Exception](f(df2))
