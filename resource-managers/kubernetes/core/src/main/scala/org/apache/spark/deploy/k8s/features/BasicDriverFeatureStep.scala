...
private[spark] class BasicDriverFeatureStep(conf: KubernetesDriverConf)
  ...
  private val memoryOverheadMiB: Long = conf
    ...
    .getOrElse(
      math.max((memoryOverheadFactor * driverMemoryMiB).toInt, driverMinimumMemoryOverhead))
  private val driverMemoryWithOverheadMiB = driverMemoryMiB + memoryOverheadMiB

  override def configurePod(pod: SparkPod): SparkPod = {
    val driverCustomEnvs =
      KubernetesUtils.buildEnvVars(Seq(ENV_APPLICATION_ID -> conf.appId) ++ conf.environment)
