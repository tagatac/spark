...
/**
 * Re-implement YARN's [[RackResolver]] for hadoop releases without YARN-9332. This also allows
 * Spark tests to easily override the default behavior, since YARN's class self-initializes the
 * first time it's called, and future calls all use the initial configuration.
 */
private[spark] class SparkRackResolver(conf: Configuration) extends Logging {
  ...
  private val dnsToSwitchMapping: DNSToSwitchMapping = {
    ...
    ReflectionUtils
      .newInstance(dnsToSwitchMappingClass, conf)
  ...
  /**
   * Added in SPARK-13704. This should be changed to `RackResolver.resolve(conf, hostNames)` in
   * hadoop releases with YARN-9332.
   */
  ...
  private def coreResolve(hostNames: Seq[String]): Seq[Node] = {
    ...
    if (rNameList == null || rNameList.isEmpty) {
      ...
      logInfo(
        log"Got an error when resolving hostNames. " +
...
/**
 * Utility to resolve the rack for hosts in an efficient manner. It will cache the rack for
 * individual hosts to avoid repeatedly performing the same expensive lookup.
 */
object SparkRackResolver extends Logging {
  @volatile private var instance: SparkRackResolver = _

  /**
   * It will return the static resolver instance. If there is already an instance, the passed conf
   * is entirely ignored. If there is not a shared instance, it will create one with the given
   * conf.
   */
