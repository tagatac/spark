...
class YarnAllocatorSuite
  ...
  def createAllocator(
    ...
    val args = Array("--jar", "somejar.jar", "--class", "SomeClass")
  ...
  test("single container allocated with ResourceProfile") {
    ...
    withResourceTypes(yarnResources) {
      ...
      handler.requestTotalExecutorsWithPreferredLocalities(
        resourceProfileToTotalExecs.toMap,
  ...
  test("multiple containers allocated with ResourceProfiles") {
    ...
    withResourceTypes(yarnResources) {
      ...
      // request a single container and receive it
      val (handler, _) = createAllocator(1)
      val resourceProfileToTotalExecs =
        mutable.HashMap(defaultRProf -> 0, rprof -> 1, rprof2 -> 2)
      val numLocalityAwareTasksPerResourceProfileId =
        mutable.HashMap(rprof.id -> 0, rprof2.id -> 0)
      handler.requestTotalExecutorsWithPreferredLocalities(
        resourceProfileToTotalExecs.toMap,
        numLocalityAwareTasksPerResourceProfileId.toMap,
        Map.empty,
  ...
  test("custom resource requested from yarn") {
    withResourceTypes(List("gpu")) {
      ...
      val expectedResources =
        Resource.newInstance(defaultResource.getMemorySize(), defaultResource.getVirtualCores)
  ...
  test("custom spark resource mapped to yarn resource configs") {
    ...
    val yarnResources =
      Seq(sparkConf.get(YARN_GPU_DEVICE), sparkConf.get(YARN_FPGA_DEVICE), yarnMadeupResource)
    withResourceTypes(yarnResources) {
      val mockAmClient = mock(classOf[AMRMClient[ContainerRequest]])
  ...
  test("gpu/fpga spark resource mapped to custom yarn resource") {
    ...
    try {
      ...
      withResourceTypes(yarnResources) {
        val mockAmClient = mock(classOf[AMRMClient[ContainerRequest]])
        val sparkResources =
          Map(EXECUTOR_GPU_ID.amountConf -> "3", EXECUTOR_FPGA_ID.amountConf -> "2")
        val (handler, _) = createAllocator(1, mockAmClient, sparkResources)
  ...
  test("some containers allocated") {
    ...
    handler.handleAllocatedContainers(
      Array(container1, container2, container3).toImmutableArraySeq)
  ...
  test("excluded nodes reflected in amClient requests") {
    ...
    val excludedNodes = Set("hostA", "hostB")
  ...
  test(
    "SPARK-28577#YarnAllocator.resource.memory should include offHeapSize " +
  ...
  test("Test YARN container decommissioning") {
    ...
    doAnswer(allocateResponseAnswer)
      .when(rmClientSpy)
      .allocate(org.mockito.ArgumentMatchers.anyFloat())

    handler.allocateResources()
    // No DecommissionExecutor message should be sent
    verify(rpcEndPoint, times(0)).send(
      DecommissionExecutorsOnHost(org.mockito.ArgumentMatchers.any()))
    ...
    handler.allocateResources()
    verify(rpcEndPoint, times(1)).send(
      DecommissionExecutorsOnHost(org.mockito.ArgumentMatchers.any()))
  ...
  test(
    "SPARK-43510: Running executors should be none when YarnAllocator adds running executors " +
    ...
    val status =
      ContainerStatus.newInstance(container.getId, ContainerState.COMPLETE, "Finished", 0)
    val getOrUpdateNumExecutorsStartingForRPId =
      PrivateMethod[AtomicInteger](Symbol("getOrUpdateNumExecutorsStartingForRPId"))
